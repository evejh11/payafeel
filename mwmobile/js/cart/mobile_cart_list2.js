...
var MobileCartList = {
   ...
   ,addEventBtn : function(){
					...
					MobileCommon.ajax({
						...
						, success : function(data, textStatus, jqXHR) {
							if(data.status == "succ") {
								...
								try {trackPurchaseClick(str,'event12'); }catch(e){}
								MobileCartList.init();
	   ...
	   $("#div_basket_list").on("mousedown",".btn_cart_delete",function(){
		   try{
			   var tr = $(this).parents(".tr_cart_list").eq(0);
	   ...
	   //장바구니 삭제
	   $("#div_basket_list").on("click",".btn_cart_delete",function(event){
		   event.preventDefault();
	   ...
	   $("#div_basket_list").on("change","select[name='i_arrProductCnt_temp']",function(event){
		   ...
		   var dayoffcd 	  = $(".span_dayoffcd",tr).text();
		   ...
		   var message	   = "";
			...
			$("input[name='i_arrProductCnt']",tr).val(obj.val());
			
			try{
				AEC_U_V(productcd,obj.val());
			}catch(e) {}
			...
			if (tr.hasClass("tr_product_list")) {
				...
				for (var i = 0; i < size; i++) {
					...
					$("*[name='i_arrProductCnt']", arrTr.eq(i)).val(fgCntPerUnit * cnt);
	   ...
	   $("#div_basket_list").on("change","select[name='i_arrOption_temp']",function(event){
		   
		    var optioncd  = $(this).val();
		    var arroption = $("input[name='i_arrOptioncd']");
		    var alreadyflag = false;
		    for(var i=0; i<arroption.size(); i++){
		    	if(arroption.eq(i).val() == optioncd){
		    		alreadyflag = true;
		    		break;
		    	}
		    }
		    var status = $("input[name='"+optioncd+"_status']").eq(0).val();
		    
		    if(alreadyflag){
		    	showMessageBox({
	    			message : "이미 동일한 옵션의 상품을 장바구니에 담았어요."
	    			,close : function(){
	    				MobileCartList.cartList();
	    				return;
	    			}
...
//	    });
		...
		$(".btn_order_choose").click(function(event) {
			event.preventDefault();
       ...
       $("#chk00").unbind("change").change(function(event){
    	   ...
    	   var len = $(".tr_product_list").find("input:checkbox").length; 
    	   if($(this).is(":checked")){
    		   chk_all.prop("checked", true);
    		   $(this).addClass("active");
    		   for(var i = 0; i < len; i++){
    			   $(".tr_product_list").find("input:checkbox").eq(i).prop("checked", true);
    			   $(".tr_product_list").find("label").eq(i).addClass("active");
    		   }
    	   }else{
    		   chk_all.prop("checked", false);
    		   $(this).removeClass("active");
    		   for(var i = 0; i < len ; i++){
    			   $(".tr_product_list").find("input:checkbox").eq(i).prop("checked", false);
    			   $(".tr_product_list").find("label").eq(i).removeClass("active");
    		   }
    	   }
       });
   ...
   ,cartList : function(flag){
				...
				if (data.status == "succ") {
					...
					if(size > 0){
						...
						for(var i=0; i<size; i++){
							...
							if(vo[i].v_prod_typecd == "PROD_0001"){
								...
								paramhtml.push("<div class='tr_product_list tr_cart_list'>");
					...
					if(productlistSize > 0){
							...
							for(var i=0; i<productlistSize; i++){
									...
									var arrHtml 	= new Array();
									var target = $(".tr_product_list").eq(i);
								   ...
								   arrHtml.push("			<input type='checkbox' id='chk"+i+"' name='i_arrChkProd001' class='checkbox' checked='checked'/>");
								   ...
								   arrHtml.push("			"+productnm.val()+"</label>");
								   arrHtml.push("		</span>");
								   arrHtml.push("	</p>");
								   ...
								   arrHtml.push("		<span class='btn_delete'><a href='#' class='btn_cart_delete'><img src='/mobile/images/common/btn_delete3.png' alt='삭제'></a></span>");
								   arrHtml.push("		<a href='"+GLOBAL_WEB_ROOT +"mobile/shop/mobile_shop_product_detail.do?i_sProductcd="+productcd.val()+"'><img src='"+productImg.val()+"' alt='' onerror='fnNoImage(this);'></a>");
								   
								   
								   var wish_seqno	= fnOnlyNumber($(".span_wish_seqno",productTr.eq(i)).text()).number;
								   var flagwish		= $(".span_flag_wish",productTr.eq(i)).text();
								   
								   if(wish_seqno > 0 && flagwish == "Y"){
									   arrHtml.push("		<a href='#balloonWish' class='ico_wish btn_delete_wish active' id='"+wish_seqno+"'><span>위시리스트 삭제</span></a>");
								   }else{
									   arrHtml.push("		<a href='#balloonWish' class='ico_wish btn_add_wish'><span>위시리스트 추가</span></a>");
								   }
									   ...
									   var unit = "원";
										...
										if(beautyFlag.val() == "Y"){
											...
											arrHtml.push("			<dt class='ttl sale'>판매금액</dt>");
											   ...
											   arrHtml.push("			<dd><em class='row_sale_price'>0</em>원</dd>");
										}
								   }else{
								   ...
								   if(optionCnt.val() > 1){
									   ...
									   arrHtml.push("			<select id='' name='i_arrOption_temp' class='selectBox3'>");
									   
										  for(var n=0; n<optioncdtemp.size(); n++){
											  
											  var option_selected = optioncdtemp.eq(n).val() == optioncd.val() ? 'selected=selected' : '';
											  
											  arrHtml.push("				<option value='"+optioncdtemp.eq(n).val()+"'"+option_selected+">");					
											  arrHtml.push("					"+optionnmtemp.eq(n).val()+"");			
									   ...
									   arrHtml.push("			</select>");
									   for(var n=0; n<optioncdtemp.size(); n++){
											arrHtml.push("<input type='hidden' name='"+optioncdtemp.eq(n).val()+"_status' value='"+optionstatustemp.eq(n).val()+"'/>");
										}
									   arrHtml.push("		</div>");
									   ...
									   if(featureTag.val().indexOf("DG_P003") > -1 || featureTag.val().indexOf("DG_P004") > -1){
										   ...
										   arrHtml.push("			<span class='ttl'>옵션</span>");
										   arrHtml.push("			<select id='' name='i_arrOption_temp' class='selectBox3'>");
										   arrHtml.push("				<option value='"+optioncd.val()+"' selected='selected'>");					
										   arrHtml.push("					"+optionnm.val()+"");			
										  ...
										  if(statuscd.val() != "0001"){
										   ...
										   arrHtml.push("		<input type='hidden' id='' name='i_arrOption_temp' value='"+optioncd.val()+"'>");
									   }
								   ...
								   for(var j=0; j<maxcartcnt; j++){
									   
									   var productCount = j+1;
									   
									   arrHtml.push("				<option value='"+productCount+"' "+(productCnt.val() == productCount ? 'selected=selected' : '')+">"+productCount+"</option>");						   
								   }
										...
										if(freegoodlistSize>0){
											...
											for(var k=0; k<freegoodlistSize; k++){
												...
												if(productKey == uproductKey){
													...
													arrHtml.push("			<div class='prodImg'>");	
													arrHtml.push("			<img src='"+freegoodImg.val()+"' alt='' onerror='fnNoImage(this);'>");
													arrHtml.push("			</div>");	
													...
													arrHtml.push("			</div>");	
													arrHtml.push("			<span class='span_fg_cnt'>"+freegoodCnt.val()+"개</span>");		
													arrHtml.push("		</li>");
	...
	, setCartPayment : function (arrChk, type) {
		...
		for (var i = 0 ; i < size; i++) {
			...
			tr = arrChk.eq(i).parents(".tr_cart_list").eq(0);
