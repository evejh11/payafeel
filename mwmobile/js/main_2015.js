...
var	MobileMain = {
	...
	init : function() {
		...
		window.nowBannerSwipe = new Swipe(document.getElementById('nowBannerSwipe'), {
			continuous: true,
			stopPropagation: true,
			callback: function(event, element) {
				$(".nowBanner-nav > span").removeClass().eq(event).addClass("active");
			}
		});
...
//						Kakao.init('8218ce208f43a5737824a66dc103d2c3');
           		    	Kakao.Story.share({
					        ...
					        text: "아모레퍼시픽몰의 스페셜 기프트 상품을 만나보세요! :)"
					    });
           		    });
                    try{
                    	Kakao.init('8218ce208f43a5737824a66dc103d2c3');
