var MobileMyChangeView ={
	name : "MobileMyChangeView"
	,init : function(){
		
		MobileMyChangeView.OrderView();
		MobileMyChangeView.addBtnEvent();
		MobileMyChangeView.isDeliveryKeyup();		
	}
	,addBtnEvent : function(){
		
		$(".a_goApp_sns").unbind("click").click(function(event){
			event.preventDefault();
			var url = "http://www.amorepacificmall.com/mobile/my/mobile_my_change_view.do?i_sOrdercd=" + $("input[name='i_sOrdercd']").val() + "&i_sFlagSnsBrowser=Y";
		...
		$(".btn_lettering_confirm").click(function(event){
			...
			MobileMyChangeView.NamingLayerPopup();
		...
		$(".btn_delivery_reg").click(function(event){
			...
			if(MobileMyChangeView.isDeliveryValidation()){
		...
		$(".btn_go_list").unbind("click").click(function(event){
			event.preventDefault();
			document.location.href = GLOBAL_WEB_URL+"mobile/my/mobile_my_change_list.do";
		});
	},
	...
	OrderView : function(){
		...
		MobileCommon.ajax({  
			url : GLOBAL_WEB_ROOT+"mobile/my/mobile_my_change_view_ajax.do"
			, type : "POST"
			, dataType : "json"
			...
			, success : function( data, textStatus, jqXHR) {
				...
				if(data.status == "succ"){
					...
					MobileMyChangeView.setPayStats(rvo, pgVo, giftcardVo, giftboxCnt);
	...
	,setPayStats : function(rvo, pgVo, giftcardVo, giftboxCnt){
		...
		$(".rvo_change_comment").text(rvo.v_comment);
...
//				arrHtml.push("</tr>");
		...
		if(rvo.v_pay_typecd == "0014"){
		
			var target = $("#tbody_admin_change_pay_info");
			var arrHtml = new Array();
