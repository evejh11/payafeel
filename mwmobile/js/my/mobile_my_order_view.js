var cancel = false;
var MobileMyOrderView ={
	name : "MobileMyOrderView"
	,init : function(){
		
		MobileMyOrderView.addBtnEvent();
		MobileMyOrderView.OrderView();
		MobileMyOrderView.isDeliveryKeyup();
	},
	addBtnEvent : function(){
		
		$(".a_goApp_sns").unbind("click").click(function(event){
			event.preventDefault();
			var url = "http://www.amorepacificmall.com/mobile/my/mobile_my_order_view.do?i_sOrdercd=" + $("input[name='i_sOrdercd']").val() + "&i_sFlagSnsBrowser=Y";

			location.href = "http://www.amorepacificmall.com/mobile/goApp.do?target="+ url;
		});
		
		//YHCHOI : 배송지 변경 
		$(".btn_addr_change").unbind("click").click(function(event){
			event.preventDefault();
		...
		$(".btn_delivery_search").click(function(event){
			event.preventDefault();
			
			var delivery_number = $(".rvo_dlvnum").text();
			
			fnDeliverySearch(delivery_number);
		});
		
		$(".btn_lettering_confirm").click(function(event){
			event.preventDefault();

			MobileMyOrderView.NamingLayerPopup();

		});
		
		$(document).on("click", ".btn_cancel_pop", function(event){
			event.preventDefault();
			//팝업열기
			modalPopup("#communityGuide");
		});
		
		$(document).on("click", ".btn_billing_cancel_pop", function(event){
			event.preventDefault();
			// 정기구독 취소팝업
		...
		$(".btn_cancel").unbind("click").click(function(event){
			event.preventDefault();
				...
				MobileCommon.ajax({
					url : GLOBAL_WEB_ROOT+"my/my_order_cancel_save.do"
					, type : "POST"
					, data : $("form[name='frm_cancel_pop']").serialize()
					, dataType : "json"
					, animation : false
					, success : function(data, textStatus, jqXHR) {
						if(data.status == "succ") {
							...
							if(IS_LOGIN) {
								modalPopupClose('#communityGuide');
		...
		$(".btn_delivery_reg").click(function(event){
			...
			if(MobileMyOrderView.isDeliveryValidation()){
		...
		$(".btn_go_list").unbind("click").click(function(event){
			event.preventDefault();
			document.location.href = GLOBAL_WEB_URL+"mobile/my/mobile_my_order_list.do";
		});
		...
		$(".btn_return").unbind("click").click(function(event){
			...
			location.href = GLOBAL_WEB_ROOT + "mobile/my/mobile_my_return_step1.do?i_sOrdercd=" + ordercd;
		...
		$(".btn_change").click(function(event){
			...
			var ordercd		=	$("input[name='i_sOrdercd']", "#frm").val();
			location.href = GLOBAL_WEB_ROOT + "mobile/my/mobile_my_change_step1.do?i_sOrdercd=" + ordercd;
		...
		MobileCommon.ajax({  
			...
			, success : function( data, textStatus, jqXHR) {
				...
				if(data.status == "succ"){
					...
					if(size > 0){
						...
						for(var i=0; i<size; i++){
							...
							if(vo[i].n_pay_bpoint > 0){
								...
								arrHtml.push("			(뷰티포인트 결제 <span class='sp_point beautytxt v2 row_save_bpoint'>"+SetNumComma(vo[i].n_pay_bpoint)+"P</span>)");
								arrHtml.push("		</p>");
							}
					...
					MobileMyOrderView.setPayStats(rvo, pgVo, giftcardVo, giftboxCnt);
					MobileMyOrderView.addBtnEvent();
				}else{
