var MobileMyCancelList ={
	name : "MobileMyCancelList"
	, i_iNowPageNo : 1
	, i_iRecordCnt : 0
	,init : function(){
		...
		if(stdt == "" && endt == ""){
			MobileMyCancelList.fnChangeDateTerm("2week");
		}else{
			MobileMyCancelList.fnMoreAjax();
		}
		
		MobileMyCancelList.addBtnEvent();
	...
	,addBtnEvent : function(){
		...
		$(".btn_search").click(function(event){
			...
			MobileMyCancelList.i_iNowPageNo = 1;
			MobileMyCancelList.i_iRecordCnt = 0;
			
			MobileMyCancelList.fnChangeDateTerm(id);
			
		});
		
		$(".btn_more_order").click(function(event) {
			event.preventDefault();
			MobileMyCancelList.fnMore();			
		});
		...
		$(".listContent").on("click",".btn_cancel_view",function(event){
			...
			MobileMyCancelList.fnCancelView(ordercd);
		});
	...
	,fnCancelView : function(ordercd,statuscd) {
		...
		try { trackClicksEx('모바일 마이파우치^주문/배송조회^취소현황','취소현황 상세', true); } catch(e){}
		frm.submit();

	}
	,fnMore : function(){
		
		var lastpage = Math.ceil(MobileMyCancelList.i_iRecordCnt/ 4);
		
		MobileMyCancelList.i_iNowPageNo++;
		
		MobileMyCancelList.fnMoreAjax();
		
		if(lastpage == MobileMyCancelList.i_iNowPageNo) {
	...
	,fnMoreAjax : function(){
		...
		MobileCommon.ajax({  
			url : GLOBAL_WEB_ROOT+"mobile/my/mobile_my_cancel_list_ajax.do"
			, type : "POST"
			, dataType : "json"
			, data : {
				  i_iNowPageNo : MobileMyCancelList.i_iNowPageNo
				, i_iRecordCnt : MobileMyCancelList.i_iRecordCnt
			...
			, success : function( data, textStatus, jqXHR) {
				...
				MobileMyCancelList.i_iRecordCnt = data.object.i_iRecordCnt;
				MobileMyCancelList.i_iNowPageNo = data.object.i_iNowPageNo;
				...
				if(size>0){
					...
					for(var i=0; i<size; i++){
						...
						arrHtml.push("				<a href='#' class='btn_cancel_view' id='"+vo[i].v_ordercd+"'>"+vo[i].v_ordercd.substring(7,20)+"</a>");
					...
					if(MobileMyCancelList.i_iNowPageNo == 1) {
					...
					var lastpage = Math.ceil(MobileMyCancelList.i_iRecordCnt == 0 ? 1 : MobileMyCancelList.i_iRecordCnt/ 10);
					...
					arrHtml.push("		<p class='sp_bg s5'>취소주문정보가 아직 없습니다.</p>");
					arrHtml.push("	</div>");
					$(".btn_more").hide();
					$(".listContent").html(arrHtml.join("")).fadeIn("slow");
