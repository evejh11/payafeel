var MobileMyChangeList ={
	name : "MobileMyChangeList"
	, i_iNowPageNo : 1
	, i_iRecordCnt : 0
	,init : function(){
		...
		if(stdt == "" && endt == ""){
			MobileMyChangeList.fnChangeDateTerm("2week");
		}else{
			MobileMyChangeList.fnMoreAjax();
		}
		
		MobileMyChangeList.addBtnEvent();
	...
	,addBtnEvent : function(){
		...
		$(".btn_search").click(function(event){
			...
			MobileMyChangeList.i_iNowPageNo = 1;
			MobileMyChangeList.i_iRecordCnt = 0;
			
			MobileMyChangeList.fnChangeDateTerm(id);
			
		});
		
		$(".btn_more_order").click(function(event) {
			event.preventDefault();
			MobileMyChangeList.fnMore();			
		});
		...
		$(".listContent").on("click",".btn_order_view",function(event){
			...
			MobileMyChangeList.fnChangeView(ordercd,statuscd);
		});
	...
	,fnChangeView : function(ordercd, statuscd) {
		...
		frm.attr("action", GLOBAL_SSL_URL + "mobile/my/mobile_my_change_view.do");
		try { trackClicksEx('모바일 마이파우치^주문/배송조회^교환현황','교환현황 상세', true); } catch(e){}
		frm.submit();

	}
	,fnMore : function(){
		
		var lastpage = Math.ceil(MobileMyChangeList.i_iRecordCnt/ 4);
		
		MobileMyChangeList.i_iNowPageNo++;
		
		MobileMyChangeList.fnMoreAjax();
		
		if(lastpage == MobileMyChangeList.i_iNowPageNo) {
	...
	,fnMoreAjax : function(){
		...
		MobileCommon.ajax({  
			url : GLOBAL_WEB_ROOT+"mobile/my/mobile_my_change_list_ajax.do"
			, type : "POST"
			, dataType : "json"
			, data : {
				  i_iNowPageNo : MobileMyChangeList.i_iNowPageNo
				, i_iRecordCnt : MobileMyChangeList.i_iRecordCnt
			...
			, success : function( data, textStatus, jqXHR) {
				...
				MobileMyChangeList.i_iRecordCnt = data.object.i_iRecordCnt;
				MobileMyChangeList.i_iNowPageNo = data.object.i_iNowPageNo;
				...
				if(size>0){
					...
					arrHtml.push("</ul>");
					if(MobileMyChangeList.i_iNowPageNo == 1) {
					...
					var lastpage = Math.ceil(MobileMyChangeList.i_iRecordCnt == 0 ? 1 : MobileMyChangeList.i_iRecordCnt/ 10);
					...
					arrHtml.push("		<p class='sp_bg s5'>교환주문정보가 아직 없습니다.</p>");
					arrHtml.push("	</div>");
					$(".btn_more").hide();
					$(".listContent").html(arrHtml.join("")).fadeIn("slow");
