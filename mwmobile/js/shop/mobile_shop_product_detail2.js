		...
		addBtnEvent : function() {
			//SNS 공유
			 $(".btn_fb").unbind("click").click(function(event){
				...
				var img = shareCon.split(",")[0];
				var name = shareCon.split(",")[1];
				var desc = shareCon.split(",")[2];
				
				var rvo = {
					 name : name
					 ,link : GLOBAL_WEB_URL+"mobile/shop/mobile_shop_product_detail.do?i_sProductcd="+$("#i_sProductcd").val()
					,picture : img
					,description : desc
				};
			...
			// 옵션 select 변경시
			$(".sel_option").unbind("change").change(function() {
			...
			$(".btn_wish_add").unbind("click").click(function(event) {
				...
				if(IS_LOGIN) {
					...
					var productnm  = $("input[name='i_sProductnm']").val();
					var brandnm  = $("input[name='i_sBrandnm']").val();
					try{
						trackPurchaseClick(brandnm+";"+productnm,'event11');
					}catch(e){}
				} 
				else {
					var options = {
...
//				showMessageBox({message : "상품의 옵션을 선택해주세요."});
            		...
            		if(isonePlusFlag){
            			...
            			showConfirmBox({
            				...
            				, ok_func : function() {
            					if(flag == "cart"){
            						try{
            							trackPurchaseClick(brandnm+";"+productnm,'scAdd');
            						}catch(e){}
		...
		setOptionSelect : function(optioncd) {
			...
			if(featureTag.indexOf("DG_P014") > -1) {
				unit = "P";
			} else {
				unit = "원";
			}
			if($("#div_option_"+optioncd).length > 0) {
				...
				$(".dd_optionMaker").text(optManufacture);
				$(".dd_optionMakecountry").text(optOrigin);
			...
			if(deliveryType == "0002" || optDeliveryType == "0002") {
				...
				noticeHtml.push("<p>* 해당상품은 <em>당일배송 상품</em>입니다. 오전 11시 이전 주문시 서울/경기 지역은 당일 상품을 받을 수 있습니다. (일부지역제외)</p>");
				noticeHtml.push("</div>");
			}
			...
			/*	if(detail.v_brandnm != undefined && detail.v_brandnm != "") {
				...
				*/
				...
				if(detail.n_list_price != detail.n_price) {
					var salePer = Math.round((parseInt(detail.n_list_price) - parseInt(detail.n_price)) * 100 / parseInt(detail.n_list_price));
				...
				if((featureTag.indexOf("DG_P015") > -1 && parseInt(detail.n_dayoff_stockqty) > 0) || featureTag.indexOf("DG_P015") == -1 ) {
				...
				if(detail.v_skin_typenm != undefined && detail.v_skin_typenm != "") {
					...
					if(featureTag.indexOf("DG_P012") > -1) {
				...
				if(circle.length >0){
					for(var k=0; k<3; k++){
						awardhtml.push("<span class=\"label_magazine\"><img src='"+GLOBAL_MOBILE_IMG_URL+"content/"+circle[k].v_tagcd+"_big.png' alt=\"\"/></span>");
					}
				}
				...
				if(option != undefined && option.length > 0 && detail.v_typecd != "0003") {
					...
					for(var i=0; i<option.length; i++) {
						if(detail.v_statuscd == "0002" || parseInt(option[i].n_stockqty) == 0 || option[i].v_opt_statuscd == "0002") {
							soldout = "(일시품절)";
							soldoutCnt++;
						} else {
							soldout = "";
						...
						if(option[i].v_tile_path != undefined && option[i].v_title_path != "") {
							colorHtml.push("<li class=\"li_color\" id=\""+option[i].v_optioncd+"\">");
							colorHtml.push("	<a href=\"#\"><img src=\""+option[i].v_tile_path+"\" /></a>");
							colorHtml.push("</li>");
						} else {
							if(option[i].v_opt_rgb != undefined && option[i].v_opt_rgb != "") {
								var arrRgb = option[i].v_opt_rgb.split(",");
								colorHtml.push("<li class=\"li_color\" id=\""+option[i].v_optioncd+"\"><a href=\"#\">");
				...
				if(freegoodList != undefined && freegoodList.length > 0) {
					var len = freegoodList.length;
					//gypark : 사은품 표시 조건 추가
					/*if(object.detail.prddetail.v_feature_tag.indexOf("특별한구성") > -1 && object.detail.option != undefined){
						var str = "";
						for(var i = 0; i < object.detail.option.length; i++){
							if(object.detail.option[i].v_flag_event == "Y"){
								str = str + ", " + object.detail.option[i].v_optionnm;
							}
						}
						
						$(".p_freeGiftMsg").html("[특별한 구성("+ str.substring(2,str.length) +")] 구매시 무료 사은품이 <em>"+freegoodList.length+"</em>개 있습니다.");
					}else if(object.detail.prddetail.v_feature_tag.indexOf("특별한구성") <0 && object.detail.option != undefined && object.detail.prddetail.v_typecd != "0003"){
						var str = "";
						for(var i = 0; i < object.detail.option.length; i++){
							if(object.detail.option[i].v_flag_event == "Y"){
								str = str + ", " + object.detail.option[i].v_optionnm;
						...
						$(".p_freeGiftMsg").html("["+str.substring(2, str.length)+"] 구매시 무료 사은품이 <em>"+freegoodList.length+"</em>개 있습니다.");
					}else if(object.detail.prddetail.v_feature_tag.indexOf("특별한구성") > -1 && object.detail.option == undefined ){
						$(".p_freeGiftMsg").html("[특별한 구성] 구매시 무료 사은품이 <em>"+freegoodList.length+"</em>개 있습니다.");
					}else{*/
						$(".p_freeGiftMsg").html("이 상품 구매시 무료 사은품이 <em>"+freegoodList.length+"</em>개 있습니다.");
					/*}*/
				...
				if(brandHot != undefined && brandHot.length > 0) {
					...
					for(var i=0; i<len; i++) {
						...
						brandHtml.push("<li>");
						brandHtml.push("	<a href=\""+GLOBAL_WEB_URL+"mobile/shop/mobile_shop_product_detail.do?i_sProductcd="+brandHot[i].v_productcd+"\">");
				...
				if(sameRecomList != undefined && sameRecomList.length > 0) {
					...
					for(var i=0; i<len; i++) {
						...
						sameHtml.push("<li>");
						sameHtml.push("	<a href=\""+GLOBAL_WEB_URL+"mobile/shop/mobile_shop_product_detail.do?i_sProductcd="+sameRecomList[i].v_productcd+"\">");
				...
				if(differentRecomList != undefined && differentRecomList.length > 0) {
					...
					for(var i=0; i<len; i++) {
						...
						differentHtml.push("	<a href=\""+GLOBAL_WEB_URL+"mobile/shop/mobile_shop_product_detail.do?i_sProductcd="+differentRecomList[i].v_productcd+"\">");
				...
				//gypark
				MobileShopProductDetail.fn.setPrdLawInfo(detail);
				...
				$(".detailDesc").attr("onclick", "trackClicksEx('apmall^" + detail.v_brandnm + " " + detail.v_productnm + " 상품상세^상품 상세설명 탭', true)");
				$(".btn_textReview").attr("onclick", "trackClicksEx('apmall^" + detail.v_brandnm + " " + detail.v_productnm + " 상품상세^구매후기 탭', true)");
				$(".btn_photoReview").attr("onclick", "trackClicksEx('apmall^" + detail.v_brandnm + " " + detail.v_productnm + " 상품상세^포토리뷰 탭', true)");
				$(".deliveryDesc").attr("onclick", "trackClicksEx('apmall^" + detail.v_brandnm + " " + detail.v_productnm + " 상품상세^배송/반품/교환 정보', true)");
				$(".cardBenefit").attr("onclick", "trackClicksEx('apmall^" + detail.v_brandnm + " " + detail.v_productnm + " 상품상세^카드사혜택 정보 탭', true)");
				$(".prdAddDesc").attr("onclick", "trackClicksEx('apmall^" + detail.v_brandnm + " " + detail.v_productnm + " 상품상세^상품 추가 정보', true)");
		...
		setPrdLawInfo: function(detail){
			...
			if(infoTypecd == "PI_001"){
				...
				prdLawHtml.push("</tr>");
				prdLawHtml.push("<tr>");
				prdLawHtml.push("		<th scope='row'><p>기능성 화장품의 경우<br>화장품법에 따른<br/>식품의약품안전처<br>심사 필 유무</p></th>");
				...
				prdLawHtml.push(" 		<td>본 제품은 질병의 예방 및 치료를 위한 의약품이 아닌 건강기능식품입니다.</td>");
				prdLawHtml.push("</tr>");
				prdLawHtml.push("<tr>");
				prdLawHtml.push("		<th scope='row'><p>유전자재조합식품 유무</p></th>");
				prdLawHtml.push(" 		<td>해당사항 없음</td>");
		...
		, setDeliveryPrice : function(isPayStats) {
			...
			if(cash_buy_limit > 0 || point_buy_limit > 0 || cash_today_limit > 0 || point_today_limit > 0) {
				...
				if($(".feature_tag").text().indexOf("DG_P014")>-1){
