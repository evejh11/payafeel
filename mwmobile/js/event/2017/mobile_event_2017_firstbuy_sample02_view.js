var MobileFirstBuySample02 = {
	name : "MobileFirstBuySample02",
	init : function(){
		MobileFirstBuySample02.fn.addBtnEvent();
	},
	fn : {
		addBtnEvent : function(){
			$(".btn_first_buy").unbind("click").click(function(event){
				...
				if(!IS_LOGIN){
					...
					MobileFirstBuySample02.fn.goBuySampleCheck();
				}
			});
			...
			$(".btn_coupon").unbind("click").click(function(event){
				...
				if(!IS_LOGIN){
					...
					MobileFirstBuySample02.fn.goCouponCheck();
				}
			});
		},
		goBuySampleCheck : function(){
			cmAjax({
				url : GLOBAL_WEB_ROOT + "mobile/event/2017/mobile_event_2017_firstbuy_sample02_check_ajax.do",
				dataType :"json",
				data : {i_sEventcd : $(".span_eventcd").text()},
				success : function(data,textStatus){
					if(data.status == "succ"){
						MobileFirstBuySample02.fn.goBuySample(data.object);
					}else{
		...
		goCouponCheck : function(){
			cmAjax({
				url : GLOBAL_WEB_ROOT + "mobile/event/2017/mobile_event_2017_firstbuy_sample_coupon02_ajax.do",
				dataType :"json",
				data : {i_sEventcd : $(".span_eventcd").text()},
				success : function(data,textStatus){
					if(data.status == "succ"){
