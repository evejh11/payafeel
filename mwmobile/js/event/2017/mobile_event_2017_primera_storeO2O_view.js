var MobilePrimeraO2O = {
	name : "MobilePrimeraO2O",
	init : function(){
		MobilePrimeraO2O.fn.addBtnEvent();
	},
	fn : {
		addBtnEvent : function(){
			...
			$("select[name='i_sStore']").change(function(event){
				event.preventDefault();
				var store = $(this).val();
				MobilePrimeraO2O.fn.goChange('D', store, '');
			});
			...
			$("select[name='i_sDay']").change(function(event){
				...
				MobilePrimeraO2O.fn.goChange('T', store, day);
			});
				...
				, success : function(data,textStatus){
					if(data.status == "succ"){
						showMessageBox({
							...
							, close : function(){
								modalPopupClose("#pop_primera_o2o");
								location.href = GLOBAL_WEB_ROOT + "mobile/event/mobile_event_view.do?i_sEventcd=EVT201708_primeraO2O";
							}	
						});
					}else{
