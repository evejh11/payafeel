...
var MobileEventView = {
	...
	fn : {
		setScrollEvent : function(){
			...
			var arr_image_wrap2 = $(".inner_prd_cont_li");

			var arr_wrap = arr_image_wrap;
			var arr_wrap2 = arr_image_wrap2;
			
			var len = arr_wrap.size();
			var len2 = arr_wrap2.size();
			if(arr_wrap.size() >0){
				for(var i=0; i<len; i++){
			...
			if(arr_wrap2.size() >0){
				for(var i=0; i<len2; i++){
					var top = arr_wrap2.eq(i).offset().top;
					if(!arr_wrap2.eq(i).hasClass("img_load_complete")){
						if (target_top > top) {
							MobileEventView.fn.getImage(arr_wrap2.eq(i));
							arr_wrap2.eq(i).addClass("img_load_complete");
						}
					}
				}
			}
		...
		addBtnEvent : function() {
			$(".btn_sul_prod").unbind("click").click(function(event){
				event.preventDefault();
				var fbobj = 
						{id : "197208744125070"
						 , tracktype : "Event_Btn"
						 , brand_ids : "sulwhasoo"
						 , contents_ids : "P00005040"};
				try{
					goReserveFacebook(fbobj);
				}catch(e){}
			});
			
			$(".btn_hera_detail").unbind("click").click(function(event){
				event.preventDefault();
				var fbobj = 
						{id : "197208744125070"
						 , tracktype : "Event_Btn"
						 , detail_depth : "Btn1_purchase"
						 , contents_ids : "SPR20170823000031948"};
				try{
					goReserveFacebook(fbobj);
				}catch(e){}
			});
			...
			$(".a_goApp_sns").unbind("click").click(function(event){
				...
				var url = "http://www.amorepacificmall.com/event/event_direct_list.do?i_sEventcd="+eventcd;
			...
			$(".btn_cart").unbind("click").click(function(event){
				...
				var flagreserve = $("input[name='i_arrFlagReserve']").eq(idx).val();
				var reservecnt = $("input[name='i_arrReserveCnt']").eq(idx).val();
				
				if(flagreserve == "Y"){
					showMessageBox({
						message : "해당 상품은 예약상품으로 <br/>상세페이지에서 바로구매만 가능해요!"
					});
				}else if(productcd == "SPR20161228000025917"){
					showMessageBox({
						message : "해당 상품은 상세페이지에서 바로구매만 가능해요!"
					});
				}else if(reservecnt > 0){
					showMessageBox({
						message : "해당 상품은 예약상품으로 <br/>상세페이지에서 바로구매만 가능해요!"
					});
				}
					...
					if($("input[name='i_sEventcd']").val() == "EVT20170828_heraRH"){
						try{
			        		var fbobj = 
							{id : "197208744125070"
							 , tracktype : "Event_Btn"
							 , detail_depth : "Btn_addtocart"
							 , contents_ids : "SPR20170823000031948"};
			        		goReserveFacebook(fbobj);
			        		goog_report_conversion(undefined,'-rFtCO_LoXQQ4LaBlgM');
			        	}catch(e){}
					}
				}
			});
			...
			$(".btn_nowBuy").unbind("click").click(function(event){
				...
				if(flagreserve == "Y"){
					...
					MobileBodyStart.addUserCart({
						list : list
						, callback : function(){
							if(productcd == "SPR20170315000028211" && $("#i_sEventcd").val() == "EVT20170320_heraBC"){
								try{
									goog_report_conversion(undefined,'HCYrCIv9onAQ4tOgyAM');
								}catch(e){}
							}
					...
					if($("input[name='i_sEventcd']").val() == "EVT20170828_heraRH"){
						try{
							...
							{id : "197208744125070"
							 , tracktype : "Event_Btn"
							 , detail_depth : "Btn2_purchase"
							 , contents_ids : "SPR20170823000031948"};
			        		goReserveFacebook(fbobj);
			        		
			        		goog_report_conversion(undefined,'3MXpCJHguXQQ4LaBlgM');
			        	}catch(e){}
					}
				}
			...
			$(".btn_goog_detail").unbind("click").click(function(event){
				event.preventDefault();
				var id = $(this).attr("name");
				goog_report_conversion(undefined , id);
			});
			...
			$(".ico_share").unbind("click").click(function(event) {
				...
				modalPopup("#modalPopupEvtShare");
                  ...
                  $(".btn_pinit").unbind("click").click(function(event) {
          				...
          				arrPars.push("url=" + encodeURI("http://www.amorepacificmall.com/event/event_direct_list.do?i_sEventcd=")+eventcd+"&i_sFlagSnsBrowser=Y");
		...
		getEventView : function() {
			MobileCommon.ajax({
				...
				data : {
					...
					, i_sEvtCatecd : $("input[name='i_sEvtCatecd']").val()
				},
				animation : false,
				success : function(data, textStatus) {
...
//						var dateChk = data.object.DATECHK;
//						var duplChk = data.object.DUPLCHK;
//						$("input[name='i_sDATECHK']").val(dateChk);
//						$("input[name='i_sDUPLCHK']").val(duplChk);
						...
						MobileEventView.fn.addBtnEvent();
						MobileEventView.fn.addBillingBtnEvent();
		...
		setEventView : function(object) {
			...
			}else if(obj.v_mobile_bg_img_path == undefined && obj.v_mobile_contents == undefined ){
				arrEvtDetail.push("<img src='"+GLOBAL_MOBILE_IMG_URL+"common/blank_640_220.png' alt='' class='w100p'>");
				arrEvtDetail.push("<div class='hashTagArea' style='background-color:#"+obj.v_evt_hex_code+";'>");
				for(var j=0; j<textlist.length; j++){
					arrEvtDetail.push("	<p class='tit "+textlist[j].v_stylecd+"'>"+textlist[j].v_text+"</p>");
				}
			...
			$("input[name='i_sOptioncd']").val(obj.v_optioncd);
			...
			if (length != undefined && length > 0) {
				...
				for (var i = 0; i < length; i++) {
					...
					if(grplist[i].v_stylecd == "CW_STYLE01"){
						if (pro_list != undefined && pro_list > 0) {
							arrr2.push("<div class='evtGroupTitle inner_prd_cont_li grp_"+grplist[i].n_grp_seqno+"'>");
							arrr2.push("<img src='"+GLOBAL_IMG_URL+"event/evt_theme_groupname_square.jpg' class='evt_group_square' />");
							arrr2.push("<span class='span_groupnm'>" + grplist[i].v_groupnm + "</span>");
							arrr2.push("<img src='"+GLOBAL_IMG_URL+"event/evt_theme_groupname_square.jpg' class='evt_group_square' />");
							arrr2.push("</div>");
							for (var j = 0; j < pro_list; j++) {
								if (pro[j].n_grp_seqno == grplist[i].n_grp_seqno) {
									...
									arrr2.push("		<a href=\"javascript:MobileBodyStart.goProductDetail('" + pro[j].v_productcd + "');\">");
									...
									if (pro[j].n_list_price > pro[j].n_price && pro[j].n_plus_evt_give_cnt <= 0) {
										...
										arrr2.push("				<span class='label_sale' style='background-color:#"+obj.v_evt_hex_code+";'>");
										
										if (pro[j].v_flag_upto == 'Y'){											
											arrr2.push("<em class='upTo'>UP TO</em>");
										}
												
										arrr2.push("<em class='str'>"+sale+"%</em><em>SALE</em></span>");
										
									}else if(pro[j].n_plus_evt_give_cnt >0 && pro[j].n_plus_evt_buy_cnt >0){
										arrr2.push("				<span class='label_sale' style='background-color:#"+obj.v_evt_hex_code+";'><em class='str'>"+pro[j].n_plus_evt_buy_cnt+"+"+pro[j].n_plus_evt_give_cnt+"</em><em>OFFER</em></span>");
									}
									if(pro[j].v_img_free_web_path != undefined && pro[j].v_img_free_web_path != "" && pro[j].v_feature_tag != undefined && pro[j].v_feature_tag != "undefined" && pro[j].v_feature_tag.indexOf("DG_P009") > -1){
										arrr2.push("				<img src='"+ GLOBAL_IMG_URL+"common/blank.png' class='img_src_change' data-chg-src='"+pro[j].v_img_free_web_path+"'alt=''>");
									}else{
									...
									if (pro[j].n_list_price > pro[j].n_price && pro[j].n_plus_evt_give_cnt <= 0) {
										arrr2.push("					<span class='sale' style='text-decoration:line-through;'>"+SetNumComma(pro[j].n_list_price)+"</span>");
									}
									...
									arrr2.push("<input type='hidden' name='i_arrFlagReserve' value='"+pro[j].v_flag_reser_real+"'/>");
									arrr2.push("<input type='hidden' name='i_arrReserveCnt' value='"+pro[j].n_reserve_cnt+"'/>");
						...
						if(i==0){
							arrr2.push("<ul class=\"promo_prodList_v2 clearfix\">");
						}else{
							arrr2.push("<ul class=\"promo_prodList_v2 clearfix\" style='margin-top:11px;'>");
						}
						if (pro_list != undefined && pro_list > 0) {
							arrr2.push("<div class='evtGroupTitle inner_prd_cont_li grp_"+grplist[i].n_grp_seqno+"'>");
							arrr2.push("<img src='"+GLOBAL_IMG_URL+"event/evt_theme_groupname_square.jpg' class='evt_group_square' />");
							...
							for (var j = 0; j < pro_list; j++) {
								if (pro[j].n_grp_seqno == grplist[i].n_grp_seqno) {
									arrr2.push("<li class='inner_prd_cont_li grp_"+grplist[i].n_grp_seqno+"'>");
									arrr2.push("	<div class='itemBox'>");
									arrr2.push("		<a href=\"javascript:MobileBodyStart.goProductDetail('" + pro[j].v_productcd + "');\">");
									arrr2.push("			<div class='thumbImg'>");
									if (pro[j].n_list_price > pro[j].n_price && pro[j].n_plus_evt_give_cnt <= 0) {
									...
									arrr2.push("				<p class='priceZone'>");
									if (pro[j].n_list_price > pro[j].n_price && pro[j].n_plus_evt_give_cnt <= 0) {
										arrr2.push("					<del>"+SetNumComma(pro[j].n_list_price)+"원</del>");
									}
									arrr2.push("					<span class='price' style='color: #"+obj.v_evt_hex_code+";'><em>"+SetNumComma(pro[j].n_price)+"</em>원</span>");
									arrr2.push("				</p>");
									arrr2.push("<input type='hidden' name='i_arrProductcd' value='"+pro[j].v_productcd+"'/>");
									arrr2.push("<input type='hidden' name='i_arrOptioncd' value='"+pro[j].v_optioncd+"'/>");
									arrr2.push("<input type='hidden' name='i_arrFlagSoloPack' value='"+pro[j].v_flag_solopack+"'/>");
				...
				$(theme.join("\n")).appendTo($(".themeSel"));
				$(".searchAreaEvent").show();
				$(".promotion_prd_cont").show();
			}else{
				$(".searchAreaEvent").hide();
				$(".promotion_prd_cont").hide();
			}
//			$(".div_event_content").html(obj.v_mobile_contents);
			...
			$(".themeSel").change(function(event){
				event.preventDefault();
				if($(".inner_prd_cont_li").size() >0){
					$(".inner_prd_cont_li").hide();
					$(".grp_"+$(this).val()).show();
					if($(this).val() == ""){
						$(".inner_prd_cont_li").show();
					}
				}
				if($(".inner_prd_cont").size() > 0){
					$(".inner_prd_cont").hide();
					$(".grp_"+$(this).val()).show();
					if($(this).val() == ""){
						$(".inner_prd_cont").show();
		...
		, addBillingBtnEvent : function(event){
			
			var restrictCnt = $(".span_restrict_cnt").text();
			...
			$("#btn_normal_purchase").unbind("click").click(function(event){
				...
				var optioncd =  $("input[name='i_sNormalOptioncd']").val();
				var productCnt = "1";
				var chkFlag = "CHK";
						...
						, success : function ( data, textStatus, jqXHR) {
							...
							if(data.status == "succ"){
								...
								if (restrictCnt <= 0){
									...
									MobileEventView.fn.addNormalPurchaseEvent(productcd, optioncd, productCnt, eventcd, evtCategorycd);
								}
							}		
						}
					});
			...
			$("#btn_billing_purchase").unbind("click").click(function(event){
				...
				var evtCategory = $("input[name='i_sEvtCatecd']").val();
				var eventcd = $("input[name='i_sEventcd']").val();
				var evtCategory = $("input[name='i_sEvtCatecd']").val();
				var productcd = $("input[name='i_sProductcd']").val();
				var optioncd =  $("input[name='i_sBillingOptioncd']").val();
				var productCnt = "1";
				var flagSoloPack = "N";
					...
					MobileCommon.ajax({
						...
						, success : function ( data, textStatus, jqXHR) {
							...
							if(data.status == "succ"){
								...
								MobileEventView.fn.addBillingPurchaseEvent(productcd, optioncd, productCnt, evtCategory, eventcd);
							} else {
		...
		addBillingPurchaseEvent : function(productcd, optioncd, cnt, evtCategorycd, eventcd){
			...
			list.push({
				...
				, optioncd : optioncd
				, cnt : cnt
				, typecd : "PROD_0001" // 확인필요
				, flagSoloPack : $("input[name='i_sFlagSoloPack']", frm_billing).val()
				, flagaction : "purchase"
			});
