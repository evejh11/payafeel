var	MobileSpecialkit2ndEvent = {
	...
	fn : {
		addBtnEvent : function(){
			$(".kit_apply").unbind("click").click(function(event){
				...
				if(!IS_LOGIN){
					...
					cmAjax({
						url : GLOBAL_WEB_ROOT+"mobile/event/2016/mobile_event_2016_specialkit_2nd_checkGrade_ajax.do"
						,data : {
					...
					cmAjax({
						...
						,success : function(data, textStatus){
							if(data.status == "succ"){
								...
								MobileSpecialkit2ndEvent.fn.finishApply(data.object);
							}else{
								if(data.object == "normal"){
									showMessageBox({
										message : "아쉽지만 고객 등급이 일치하지 않습니다.<br/>고객님의 등급을 확인 후 신청해주세요."
			...
			$(".confirm_myInfo").unbind("click").click(function(event){
				...
				if(!IS_LOGIN){
					...
					cmAjax({
						url : GLOBAL_WEB_ROOT + "mobile/event/2016/mobile_event_2016_specialkit_2nd_confirm_myinfo.do"
		...
		finishApply : function(kit){
			cmAjax({
				url : GLOBAL_WEB_ROOT+"mobile/event/2016/mobile_event_2016_specialkit_2nd_save_ajax.do",
				type: "post",
				data: {	i_sEventcd : $(".span_evtcode").text()
							,i_sFlagKit : kit 
							,i_sFlagAction : "R"
				...
				success : function(data,textStatus,jqXHR){
					if(data.status == 'succ'){
						showMessageBox({
							message : "짝짝짝! 멤버십 키트 신청이 완료 되었습니다.<br>키트는 <strong>8월 16일</strong> 이후 일괄 배송될 예정입니다."
							, close : function(){
