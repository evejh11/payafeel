var MobileTimeAttack = {
	...
	fn : {
		addBtnEvent : function(){
			$(".btn_coupon1").unbind("click").click(function(event){
				...
				if(!IS_LOGIN){
					...
					cmAjax({
						url : GLOBAL_WEB_ROOT + "mobile/event/2016/mobile_event_2016_time_attack_check_ajax.do",
						dtatType :"json",
						...
						success : function(data,textStatus){
							if(data.status == "succ"){
								MobileTimeAttack.fn.getCoupon("TENCP",data.object);
							}else{
			...
			$(".btn_coupon2").unbind("click").click(function(event){
				...
				if(!IS_LOGIN){
					...
					cmAjax({
						...
						success : function(data,textStatus){
							if(data.status == "succ"){
								MobileTimeAttack.fn.getCoupon("TWOCP",data.object);
							}else{
								if(data.object == "login"){
									showConfirmBox({
										message : "로그인 하시면 서비스 이용이 가능하세요!"
		...
		getCoupon : function(flag, couponcd){
			cmAjax({
				url : GLOBAL_WEB_ROOT + "mobile/event/2016/mobile_event_2016_time_attack_save_ajax.do",
				dataType : "json",
				data : {i_sEventcd : $(".span_eventcd").text(),
						i_sCouponFlag : flag,
