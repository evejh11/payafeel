var	MobileSpecialkit1stEvent = {
	name : "MobileSpecialkit1stEvent",
	init : function() {
		MobileSpecialkit1stEvent.fn.addBtnEvent();
	},
	fn : {
		addBtnEvent : function(){
			$(".kit_apply").unbind("click").click(function(event){
				...
				if(!IS_LOGIN){
					...
					cmAjax({
						url : GLOBAL_WEB_ROOT+"mobile/event/2016/mobile_event_2016_specialkit_1st_checkGrade_ajax.do"
						,data : {
			...
			$(".confirm_myInfo").unbind("click").click(function(event){
				...
				if(!IS_LOGIN){
					...
					cmAjax({
						url : GLOBAL_WEB_ROOT + "mobile/event/2016/mobile_event_2016_specialkit_1st_confirm_myinfo.do"
			...
			$(".btn_complete").unbind("click").click(function(event){
				event.preventDefault();
				cmAjax({
					url : GLOBAL_WEB_ROOT+"mobile/event/2016/mobile_event_2016_specialkit_1st_save_ajax.do",
					type: "post",
					data: {	i_sEventcd : $(".span_evtcode").text()
					...
					success : function(data,textStatus,jqXHR){
						if(data.status == 'succ'){
							...
							modalPopupClose("#modalPopupSpecialVVIPkit");
							showMessageBox({
								message : "짝짝짝! 멤버십 키트 신청이 완료 되었습니다.<br>키트는 <strong>2월 1일</strong> 이후 일괄 배송될 예정입니다."
								, close : function(){
