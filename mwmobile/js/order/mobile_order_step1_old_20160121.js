...
var MobileOrderDelivery = {
	...
	,addBtnEvent : function(){
		...
		$(".btn_delivery_modify").click(function(event){
			...
			if (addresscd == "") {
				showMessageBox({message : "변경할 배송지를 선택해 주세요"});
				return;
			}
...
var MobileOrderCoupon = {
	...
	,addBtnEvent : function(){
		...
		$(".btn_user_coupon_pop").unbind("click").click(function(event){
			...
			if (!MobileOrderStep1.isLogin) {
				showMessageBox({message : "쿠폰사용은 로그인한 회원만 사용이 가능합니다."});
				return;
			}
		...
		$(".btn_coupon_reg").click(function(event) {
			...
			if (!MobileOrderStep1.isLogin) {
				showMobileMessageBox({message : "쿠폰사용은 로그인한 회원만 사용이 가능합니다."});
				return;
			}
	...
	, couponReg : function() {
		...
		if(isEmpty($("input[name='i_sCertifyCode']").val())) {
			showMessageBox({message : "쿠폰번호를 입력해주세요."});
			return;
		}
			...
			, dataType : "json"
				...
				, success : function(data, textStatus, jqXHR) {
					if(data.status == "succ") {
						...
						showMessageBox({message : "쿠폰이 등록되었습니다."});
	...
	, fnApply : function () {
		...
		if (size1 == 0) {
			...
			showMessageBox({message : "적용할 쿠폰이 없습니다."
				,close : function(){
		...
		if (size2 == 0) {
			...
			showMessageBox({message : "사용할 쿠폰을 선택해 주세요"
				,close : function(){
					$("<div class='overlay'></div>").appendTo("#wrap");
				}
			});
		...
		for (var i = 0; i < size2; i++) {
			...
			if (v_typecd == "0001" || v_typecd == "0005") {
				...
				if (v_productcd == undefined || v_productcd == "") {
					...
					showMessageBox({message : "적용할 상품을 선택해 주세요"
						,close : function(){
				...
				if(v_typecd != "0005"){
					if(MobileOrderCoupon.productCntCheck(v_productcd,v_flag_option,v_typecd)){
						...
						showMessageBox({message : "1개의 상품에 1개의 쿠폰만 적용됩니다"
						,close : function(){
								$("<div class='overlay'></div>").clone().appendTo("#wrap");
								MobileOrderCoupon.productCntCheck("","N","");
		...
		if(isDisPriceFlag){
			...
			showMessageBox({message : "이미 할인된 가격이 있는 상품이 선택되어 있습니다."
				,close : function(){
					$("<div class='overlay'></div>").appendTo("#wrap");
					MobileOrderCoupon.productCntCheck("","N","");
				}
					...
					, success : function ( data, textStatus, jqXHR) {
						...
						if(data.status == "succ"){
							...
							if ($(".tr_order_coupon_freegood").size() == 0) {
								...
								showMessageBox({message : "죄송합니다. 지급하는 상품이 현재 품절 상태 입니다."});
								return;
   ...
   ,addBtnEvent : function (){
	   ...
	   $(".btn_giftbox_confirm").click(function(event){
		   ...
		   if(cnt == 0){
			   ...
			   showMessageBox({message : "신청한 선물포장 서비스가 없습니다."});
			   return;
	   ...
	   $("input[name='i_sFlagGiftbox_temp']").click(function(event){
			...
			if($(this).prop("checked")){
				...
				showConfirmBox({
					message : "신청하신 선물포장 서비스를 취소하시겠습니까?"
					,ok_func : function(){
...
var MobileOrderNaming = {
	...
	addBtnEvent : function(){
		...
		$("input[name='i_sFlagNaming_temp']").click(function(event){
			...
			if($(this).prop("checked")){
				...
				showConfirmBox({
					message : "신청하신 레터링 서비스를 취소하시겠습니까?"
					,ok_func : function(){
		...
		$(".btn_naming_confirm").click(function(event){
			...
			if(MobileOrderNaming.naming_cnt == 0){
				...
				showMessageBox({message : "신청한 레터링 서비스가 없습니다."});
	...
	, fnApply : function () {
		...
		if(cnt == 0){
			...
			showMessageBox({message : "상품이 선택되지 않았거나 위치가 설정되어 있지 않아서 신청이 불가능합니다."});
			return;
			
		}
		
		if(!flagsize){
			showMessageBox({message :"국문 10자, 영문 20자 내에서만 신청이 가능합니다."});
			return;
		}
		
		if(messagecnt == 0){
			
			showMessageBox({message : "문구가 제대로 적혀 있지 않아서 신청이 불가능합니다."});
			return;
		}
...
var MobileOrderGiftCard = {
	...
	,addBtnEvent : function(){
		...
		$(".btn_giftcard_search").click(function(event){
			...
			if (MobileOrderGiftCard.balance == 0) {
				showMessageBox({message : "사용가능한 기프트카드가 없습니다."});
				return;
			}
				...
				MobileCommon.ajax({
					...
					, success : function ( data, textStatus, jqXHR) {
						if (data.status == "succ") {
							...
							arrHtml.push("	<dt>기프트카드(<span id='span_giftcard_cnt'>"+MobileOrderGiftCard.use_cnt+"</span>) 사용금액</dt>");
	...
	,isGiftcardApplyValidate : function(){
		...
		if (!MobileOrderGiftCard.isRowPayment) {
			...
			showMessageBox({ message: "사용금액이 사용가능금액보다 큽니다."});
		...
		if (sum_pay_price < MobileOrderGiftCard.payment_total) {
			...
			showMessageBox({ message : "주문금액보다 기프트카드 사용금액이 더 많습니다.\n" + SetNumComma(sum_pay_price) + "원 이하로 입력해 주세요."});
...
var	MobileOrderStep1 = {
	...
	,setPaymentSetting : function(){
		var value = $("select[name='i_sPayTypecd']").val();
		...
		if(value == "0001"){
			...
			$("#tr_escrow_list").hide();
			$("#tr_taxbill_num").hide();
			$("#tr_receipt_choose").hide();
			$("#tr_bank_list").hide();
			$("#paylastday").hide();
			$("#tr_payco_list").hide();
			$("select[name='i_sPayCardTypecd']").attr("disabled",false);
			
			//MobileOrderStep1.orderCodeList(value);
			...
			$("#paylastday").hide();
			$("#tr_payco_list").hide();
			$("select[name='i_sPayCardTypecd']").attr("disabled",true);
			
		}else if(value == "0003"){
			
			$("#tr_receipt_choose").show();
			$("#tr_card_list").hide();
			$("#tr_kakao_list").hide();				
			$("#tr_escrow_list").show();
//			$("#tr_taxbill_num").show();
...
//				target1.attr("disabled",true);
		...
		$("select[name='i_sPayTypecd']").change(function(event){
			...
			if(value == "0001"){
				...
				var receiptType = $("input[name='i_sCashReceiptType']").val();
				
				if (receiptType == 'P') {
					$("#cash01").prop("checked", true);
					$("#tr_taxbill_num").show();
				} else if (receiptType == 'C') {
					$("#cash02").prop("checked", true);
					$("#tr_taxbill_num").show();
				} else if (receiptType == 'N') {
					$("#cash05").prop("checked", true);
				}
			} else if(value == "0021"){
		...
		$(".btn_payment").click(function(event) {
			...
			if(!MobileOrderStep1.isLogin){
				...
				if (!isPassMobile && mobile1 == "") {
					showMessageBox({
						message:"주문자 휴대전화를 입력해 주세요"
						,close : function(){
							$("select[name='i_sOrderMobile1']").focus();
						}
					});
					addWebErrorMessageBox($("*[name='i_sOrderMobile1']", frm),"주문자 휴대전화를 입력해 주세요");
					return;
				...
				if (!isPassMobile && (mobile2 == "" || mobile2.length < 3)) {
					...
					addWebErrorMessageBox($("*[name='i_sOrderMobile2']", frm),"주문자 휴대전화를 입력해 주세요");
					return;	
				...
				if (!isPassMobile && (mobile3 == "" || mobile3.length < 4)) {
					...
					addWebErrorMessageBox($("*[name='i_sOrderMobile3']", frm),"주문자 휴대전화를 입력해 주세요");
					return;
				}
				...
				MobileCommon.ajax({
					...
					, success : function ( data, textStatus, jqXHR) {
						...
						if(data.status == "succ"){
							...
							if(data.object >= 3){
								showMessageBox({message : "1일 최대 주문횟수를 초과하였습니다.</br> (ID당 1일 3회 주문 가능)"});
								return;
							} else {
								...
								if($("input[name='i_arrBfreeGoodChk']").size() > 0){
									...
									if(gift_size == 0){
										...
										showConfirmBox({
											message : "구매 금액대별 사은품을 선택하지 않으셨습니다. 계속 진행하시겠습니까??"
											,ok_func : function(){
...
//			window.open(orderurl, 'popupPayco', 'top=100, left=300, width=727px, height=512px, resizble=no, scrollbars=yes');
	...
	, actionPayment : function () {
		...
		if(!isBpointPoructFlag){
			showMessageBox({message :"["+productnm+"] 상품은 뷰티포인트로만 구매 가능한 상품이 있습니다. 뷰티 포인트를 사용해 주시기 바랍니다."});
			return;
		}
			...
			, success : function (data, textStatus, jqXHR) {
				if (data.status == "succ") {
					...
					if (MobileOrderStep1.sum_pay_money > 0) {
						...
						showMessageBox({message : "결제할 상품이 없습니다."});
	...
	, isValidation : function () {
		...
		if (prodCnt == 0 && MobileOrderStep1.sum_pay_mpoint == 0) {
			showMessageBox({message : "주문할 상품이 없습니다."});
			return false;
		}
		else if (prodCnt == 0 && MobileOrderStep1.sum_pay_mpoint > 0) {
			showMessageBox({message : "본품 결제액 5,000원 이상 구매시만<br/>블루리본 포인트 상품을 구매 하실수 있습니다."});
			return false;
		}
		...
		if (!isResult) {
			showMessageBox({message : "필수 입력값을 확인해 주세요."});
			target.focus();						
		}
	...
	, isValidationOrderUser : function () {
		...
		if ($(".span_sold_out").size() > 0) {
			...
			showMessageBox({message : "죄송합니다.<br/><br/>일시품절 혹은 재고부족 상품이 있어<br/>주문이 불가능 합니다."});
			return;
	...
	, isValidationEtc : function () {
		...
		if (MobileOrderStep1.sum_pay_money > 0) {
			...
			if (payType == undefined || payType == "") {
				...
				if(!($("input:radio[name='i_sFlagEscrowYn']").eq(0).prop("checked") || $("input:radio[name='i_sFlagEscrowYn']").eq(1).prop("checked"))){

					addErrorMessageBox($("*[name='i_sFlagEscrowYn']", frm).eq(0), "에스크로 결제 여부를 선택해주세요.", "");
					isResult = false;
					if (target == undefined) {
						...
						$("input:checkbox[name='i_arrBfreeGoodChk']").unbind("click").click(function(event){
							...
							if($(this).prop("checked")){
								if(chooseCnt >= choiceCnt){
									...
									showMessageBox({message : "선택 할 수 있는 사은품 갯수가 초과했습니다."});
									$(this).prop("checked",false);
									return;
	...
	,OrderList : function(){
			...
			if(productlistSize > 0){
					...
					for(var i=0; i<productlistSize; i++){
							...
							if((statuscd.val() != "0001" &&  statuscd.val() != "0005") || stockqty.val()<=0 ){
							...
							if(productcd.val() == "SPR20151223000014028"){
					        	disabeld = "disabled";
					        }
									...
									for(var k=0; k<freegoodlistSize; k++){
										...
										if(productkey == UproductKey){
											...
											if(productkey_temp != UproductKey){
												...
												arrGift.push("			<p class=\"freeGift_bubble\">이 제품 구매시 사은품 증정!</p>");
												arrGift.push("		</div>");
												arrGift.push("	</div>");
												arrGift.push("</div>");
	...
	, sum : function (flag) {
		...
		for (var i = 0; i < size; i++) {
			...
			if (flag_bpoint == "Y") {
				...
				if (!MobileOrderStep1.isBpointSetting(tr.eq(i), feild.eq(i), sum_pay_bpoint)) {
					
					showMessageBox({message : "뷰티포인트 잔액이 부족합니다.",
						close : function(){

							var el_checkbox = $("input[name='i_arrFlagBpoint_temp']", feild.eq(i));
							el_checkbox.prop("checked", false);
							
							MobileOrderStep1.setBpointCheckEvent(tr.eq(i),feild.eq(i));
						}
					});
			...
			if(isonePlusFlag){
				...
				if("Y" == $("input[name='i_arrFlagBeauty']", tr.eq(i)).val()){
					...
					if(dayoffStockqty > 0 && dayoffcd !=""){
						...
						var todaydt = $("#i_sTodayDt").val();
						if((todaydt >= 20151001 &&  dis_price > 0) && dis_cp_prod_cnt == 0){
...
//				MobileOrderStep1.sum_dis_coupon_price = MobileOrderStep1.delivery_price; 
		...
		if (!cpobj.isUsedAll) {
			...
			for (var i = 0; i < size_temp; i++) {
				...
				if(cpobj.data.v_flag_pay_money == "M" && (dis_price >= cpobj.data.n_pay_money || dis_price2 >= cpobj.data.n_pay_money)){
					is_high_saleFlag = true;
				}
				if(cpobj.data.v_flag_pay_money == "P" && (dis_per >= cpobj.data.n_pay_money || dis_per2 >= cpobj.data.n_pay_money)){
					is_high_saleFlag = true;
				}
			}				
			
			if(is_trFlag){
				message = "[" + cpobj.data.v_couponnm + "]은 이미 할인된 상품에는 적용되지 않습니다.";
			}else if(is_trSetFlag){
				message = "[" + cpobj.data.v_couponnm + "]은 스폐셜세트에 적용되지 않습니다.";
			}else if(is_high_saleFlag){
				message = "[" + cpobj.data.v_couponnm + "]은<br/> 현재 할인가로 구매하시는것이 <br/>더욱 알뜰한 쇼핑이에요!";
			}else{
				message = "[" + cpobj.data.v_couponnm + "]을(를) 사용할 수 없습니다.";
			}
	...
	, setPayStats : function () {
		...
		$(".sum_price").text(SetNumComma(""+MobileOrderStep1.sum_price));						 // 주문금액
