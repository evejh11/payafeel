...
var MobileOrderUserInfo = {
		...
		,nonmemberSetting : function(){
			   ...
			   if(!MobileOrderBillingStep1.isLogin){
...
var MobileOrderCoupon = {
	...
	,addBtnEvent : function(){
		...
		$("#div_coupon_list").on("click","input[name='i_arrCheckbox']",function(event) {
			...
			if ($(this).prop("checked")) {
				...
				if(type == "0003"){
					MobileOrderBillingStep1.flag_use_delivery_coupon = "N";
				}
				tr_prod.hide();
		...
		$("#dl_coupon_list").on("click",".btn_delete_coupon",function(event) {
			...
			MobileOrderBillingStep1.sum("clear");
		});
		/**gypark : AP몰 개선 - 전화번호 입력시, 숫자가 아니면 alert창 뜨게 함 [s] */
	...
	, clearCoupon : function (flag) {
		...
		MobileOrderBillingStep1.sum(flag);
	}
	...
	//YHCHOI : 이 부분도 수정이 필요함. MobileOrderBillingStep1.sum() 부분에서 나중에 처리하도록 바꾸자.
	...
	, showUserCouponPopup : function () {
		...
		arrParam.push("i_iSumPayPrice=" + MobileOrderBillingStep1.sum_pay_price);
		arrParam.push("i_iDeliveryPrice=" + MobileOrderBillingStep1.delivery_price);
		arrParam.push("i_sTargetDtm=" + MobileOrderBillingStep1.getPageParam().targetDt);
	...
	, getCheckCouponUsedAll : function () {
		...
		var size = MobileOrderCoupon.list == undefined ? 0 : MobileOrderCoupon.list.length;
		var pay_price = MobileOrderBillingStep1.sum_pay_price + MobileOrderBillingStep1.sum_dis_coupon_price;
	...
	, setApplyCouponCheck : function() {
		...
		pay_price = MobileOrderBillingStep1.sum_pay_price;
...
//					}
		...
		if(MobileOrderBillingStep1.delivery_price > 0){
			var tr = $(".tr_coupon_list");
			for(var h=0; h<tr.size(); h++){
				var tr_list = tr.eq(h);
				if($(".span_typecd",tr_list).text() == "0003"){
					var pay_price 		= MobileOrderBillingStep1.sum_pay_price;
			...
			for(var h=0; h<tr.size(); h++){
				var tr_list = tr.eq(h);
				if($(".span_typecd",tr_list).text() == "0003"){
					if(MobileOrderBillingStep1.flag_use_delivery_coupon == "N"){
						$("input[name='i_arrCheckbox']",tr_list).prop("disabled",false);
						$("input[name='i_arrCheckbox']",tr_list).prop("checked",false);
						var checkbox =$(".inputChk",tr_list); 
						checkbox.addClass("disabled");
...
var MobileOrderGiftBox = {
   ...
   ,addBtnEvent : function (){
	   ...
	   $("#div_giftbox_list").on("click",".btn_giftbox_apply",function(event){
		  ...
		  //YCHOI : 이 부분 나중에 MobileOrderBillingStep1.sum() 함수로 옮겨야 한다. 아직 어떻게 적용할지 확정되지 않아서 보류
   ...
   , setGiftboxPrice : function ( isPayStats, packingFlag ) {
		...
		if ($("input[name='i_sFlagGiftbox_temp']", frm).prop("checked")) {
			...
			if(packingFlag != "F"){
				...
				if (MobileOrderDelivery.gift_box_limit > 0 && MobileOrderBillingStep1.sum_pay_price >= MobileOrderDelivery.gift_box_limit  ) {
					MobileOrderBillingStep1.gifgbox_price = 0;
				}
				else {
					MobileOrderBillingStep1.gifgbox_price = MobileOrderDelivery.gift_box;	
				}
			}
		...
		if (isPayStats) {
			MobileOrderBillingStep1.setPayStats();
		}
	}
...
var MobileOrderNaming = {
						...
						MobileCommon.ajax({
							...
							, success : function ( data, textStatus, jqXHR) {
								if (data.status == "succ") {
									...
									MobileOrderBillingStep1.setNamingPrice("",0,0);
	...
	,setNamingStatus : function(parentObj){
		...
		MobileOrderNaming.sum_pay_price = MobileOrderBillingStep1.sum_pay_price;
		...
		MobileCommon.ajax({
			...
			, success : function ( data, textStatus, jqXHR) {
				if (data.status == "succ") {
					...
					MobileOrderBillingStep1.setNamingPrice(data.object, cnt, cnt  * MobileOrderNaming.naming_price);
...
var MobileOrderGiftCard = {
				...
				MobileCommon.ajax({
					...
					, success : function ( data, textStatus, jqXHR) {
						if (data.status == "succ") {
							...
							arrHtml.push("	<dd><span id='span_giftcard_payment'>"+MobileOrderGiftCard.payment_total+"</span>원<a href='#' class='btn_delete btn_gift_delete'><img src='"+GLOBAL_MOBILE_IMG_URL+"common/btn_delete2.png' alt='삭제'></a></dd>");
							arrHtml.push("</dl>");
	...
	, setPayGiftcard : function (giftcardTempcd, payGiftcard) {
		...
		MobileOrderBillingStep1.sum_pay_giftcard = payGiftcard;
		MobileOrderBillingStep1.setPayStats();
	}
	...
	,isGiftcardApplyValidate : function(){
		...
		var sum_pay_price = MobileOrderBillingStep1.sum_pay_price - MobileOrderBillingStep1.sum_dis_channel_price;
...
var	MobileOrderBillingStep1 = {
	name : "MobileOrderBillingStep1"
	, status : ""
	, isLogin : false
	, isSnsLogin : false
	, sum_price : 0
	...
	, init : function(){
		...
		MobileOrderBillingStep1.isLogin = $("input[name='i_sLoginFlag']").val() == "" ? false : true;
		MobileOrderBillingStep1.isSnsLogin = $("input[name='i_sSnsLoginFlag']").val() == "" ? false : true;
		
		MobileOrderBillingStep1.channel_dc_per = fnOnlyNumber($("input[name='i_iChannelDcPer']").val()).number; 
		MobileOrderBillingStep1.channel_dc_brd_exc = $("input[name='i_sChannelDcBrdExc']").val();
		...
		MobileOrderBillingStep1.OrderList();
		MobileOrderBillingStep1.sum();
		MobileOrderBillingStep1.addBtnEvent();
		
		MobileOrderUserInfo.nonmemberSetting();
	
		MobileOrderBillingStep1.settingAccotit();
		
		MobileOrderBillingStep1.isValidationKeyup();
		
		MobileOrderBillingStep1.setPaymentSetting();
		...
		if($("#i_sFlagBeautyHotdeal").val() == "Y" && $("#i_sBeautyHotdealcd").val() != undefined && $("#i_sBeautyHotdealcd").val() != ""){
			MobileOrderBillingStep1.flag_beauty_hotdeal = "Y";
...
//				$("checkbox[name='i_sTaxBillSaveYn']").attr("disabled",true);	//무통장 현금영수중
		...
		$("input[name='i_arrFlagBpoint_temp']").click(function(event) {
			...
			if(productcd == "SPR20160201000015368" || productcd == "SPR20160125000015130" || productcd == "SPR20160308000016415" || productcd == "SPR20160215000015629"){
				...
				if ($(this).prop("checked")) {
					...
					if(MobileOrderBillingStep1.flag_beauty_hotdeal == "Y"){
				...
				MobileOrderBillingStep1.sum();
				if(MobileOrderBillingStep1.flag_beauty_hotdeal != "Y"){
					var prev_pay_bpoint = MobileOrderUserInfo.bpoint - MobileOrderBillingStep1.sum_pay_bpoint;
		...
		$(".btn_payment").click(function(event) {
			event.preventDefault();
			if(!MobileOrderBillingStep1.isLogin){
				showMessageBox({message : "로그인이 필요한 서비스 입니다."});
				return;
			}else{
				MobileCommon.ajax({
								...
								if($("input[name='i_arrBfreeGoodChk']").size() > 0){
									...
									if(gift_size == 0){
										...
										showConfirmBox({
											...
											,ok_func : function(){
												MobileOrderBillingStep1.actionPayment();						
											},cancel_func : function(){
	...
	, lguplusBillingCall : function(vo){
		...
		$("input[name='LGD_TXNAME']").val("CardAuth");
		...
		$("input[name='KVP_QUOTA_INF']").val(vo.quotatypecd); //ISP 할부기간 표시
		
		vo.paytypecd = "SC0010";
		
		$("input[name='LGD_CARDTYPE']").val(vo.subtypecd); //카드종류
		...
		//WHNHO : BillingKey요청화면 호출
		var frm_billkey = $("form[name='frm_billkey']");
		var userBillkey = trim($("input[name='i_sUserBillkey']", frm_billkey).val());
		
		
		var frm_billkey = $("form[name='frm']"); 
		var evtCatecd = ($("input[name='i_sBillingEvtCategorycd']", frm).val());
		var eventcd = ($("input[name='i_sBillingEventcd']", frm).val());
		
		if (userBillkey == 'N'){
			
			var frm_lguplus_billing = $("form[name='frm_lguplus_billing']");
			
			
			
			$("input[name='CST_PLATFORM']", frm_lguplus_billing).val(vo.cstplatform);
			$("input[name='CST_MID']", frm_lguplus_billing).val(vo.cstmid);
			$("input[name='LGD_WINDOW_TYPE']", frm_lguplus_billing).val("submit");//수정불가
			$("input[name='LGD_MID']", frm_lguplus_billing).val(vo.lgdmid);
			$("input[name='LGD_BUYERSSN']", frm_lguplus_billing).val("");
			$("input[name='LGD_CHECKSSNYN']", frm_lguplus_billing).val("N");
			$("input[name='LGD_RETURNURL']", frm_lguplus_billing).val(GLOBAL_SSL_URL + "mobile/order/mobile_order_billkey_return.do?i_sOrdercd=" + vo.ordercd + "&i_sEvtCatecd=" + evtCatecd + "&i_sMseqno=" + vo.mseqno + "&i_sEventcd=" + eventcd);
			$("input[name='LGD_PAYWINDOWTYPE']", frm_lguplus_billing).val("CardBillingAuth_smartphone");
			//$("input[name='LGD_CUSTOM_SWITCHINGTYPE']", frm_lguplus_billing).val("IFRAME");
			//$("input[name='LGD_INSTALL']", frm_lguplus_billing).val("00");
			$("input[name='LGD_VERSION']", frm_lguplus_billing).val("JSP_SmartXPay_CardBilling");
			//return;
			//alert(1);
			
			$("input[name='CST_PLATFORM']", frm_lguplus_billing).val("service");
			$("input[name='CST_MID']", frm_lguplus_billing).val("apmall_auto16");
			$("input[name='LGD_MID']", frm_lguplus_billing).val("apmall_auto16");
			
			
			//lgdwin = open_paymentwindow(document.getElementById('frm_lguplus_billing'), vo.cstplatform, "submit");
			lgdwin = open_paymentwindow(document.getElementById('frm_lguplus_billing'), "service", "submit");
			
			//lgdwin = openXpay(document.getElementById('frm_lguplus_billing'), 'test', 'iframe', null, "", "");
		} else {
			showConfirmBox({
				message : "등록하신 신용카드로 결제가 진행됩니다. 계속 하시겠어요?"
				,ok_func : function(){
					
					var frm_lguplus = $("form[name='frm_lguplus']");
					
					$("input[name='LGD_TXNAME']", frm_lguplus).val("CardAuth");
					$("input[name='LGD_PAN']", frm_lguplus).val(userBillkey);
					$("input[name='LGD_INSTALL']", frm_lguplus).val("00");
					$("input[name='VBV_ECI']", frm_lguplus).val("010");
...
//			window.open(orderurl, 'popupPayco', 'top=100, left=300, width=727px, height=512px, resizble=no, scrollbars=yes');
	...
	, naverCall : function(vo){
		...
		if(vo.code == "Success"){
			...
			showMessageBox({message : vo.resultmsg
				,close : function(){
					WebOrderStep1.setPaymentStatus("");
				}
			});
	...
	, actionPayment : function () {
		...
		// 결제 진행중
		if (MobileOrderBillingStep1.status != "") {
			return;
		}
		
		if (!MobileOrderBillingStep1.isValidation()) {
			return;
		}
		...
		MobileOrderBillingStep1.setPaymentStatus("ING");
		...
		MobileCommon.ajax({
			...
			, success : function (data, textStatus, jqXHR) {
				if (data.status == "succ") {
					...
					MobileCommon.ajax({
						url : GLOBAL_WEB_ROOT + "mobile/order/mobile_order_billing_step1_save.do"
						, type : "POST"
						, data : frm.serialize()
						, dataType : "json"
						,animation	: false
						, success : function (data, textStatus, jqXHR) {
							if (data.status == "succ") {
								...
								if (MobileOrderBillingStep1.sum_pay_money > 0) {
									
										//MobileOrderBillingStep1.lguplusCall(vo);
										MobileOrderBillingStep1.lguplusBillingCall(vo);
								}
								
								else if (MobileOrderBillingStep1.sum_pay_bpoint > 0 || MobileOrderBillingStep1.sum_pay_mpoint > 0 || MobileOrderBillingStep1.sum_pay_giftcard > 0) {
	...
	, isValidation : function () {
		...
		if(!$("input[name='i_sAgreeAutoPay']").prop("checked")){
			showMessageBox({message : "자동결제 됨에 동의해야 결제 하실 수 있습니다."});
			return false;			
		}
		
		if(!$("input[name='i_sAgreeBilling']").prop("checked")){
			showMessageBox({message : "정기구독서비스 신청내역에 동의해야 결제 하실 수  있습니다."});
			return false;			
		}
		
		if (prodCnt == 0 && MobileOrderBillingStep1.sum_pay_mpoint == 0) {
			showMessageBox({message : "주문하실 상품을 선택해주세요."});
			return false;
		}
		else if (prodCnt == 0 && MobileOrderBillingStep1.sum_pay_mpoint > 0) {
		...
		if(isResult){
			...
			resObj = MobileOrderBillingStep1.isValidationOrderUser();
		...
		if(isResult){
			...
			resObj = MobileOrderBillingStep1.isValidationDeliveryUser();
			isResult = resObj.isResult;
...
//		if(isResult){
//
//			resObj = MobileOrderBillingStep1.isValidationEtc();
//			isResult = resObj.isResult;
//			
//			target = resObj.target;
//		}
...
//		}
	...
	,GiftList : function(flag){
		...
		for(var i=0; i<soloSize; i++){
		    ...
		    var i_iPayPriceSum = MobileOrderBillingStep1.sum_pay_price - i_iSoloPayPriceSum;
...
//								arrHtml003.push("			</span>");
						...
						if(betweencnt == 0 && specialcnt == 0 && MobileOrderBillingStep1.vip_pointmall_cnt == 0 && MobileOrderBillingStep1.rebon_pointmall_cnt == 0 ){
	...
	,OrderList : function(){
			...
			if(productlistSize > 0){
					...
					for(var i=0; i<productlistSize; i++){
							...
							arrHtml.push("</div>");
							arrHtml.push("</div>");
							var disabeld = isonePlusFlag || !MobileOrderBillingStep1.isLogin ? 'disabled' : '';
							
							if(productcd.val() == "SPR20160201000015368" || productcd.val() == "SPR20160125000015130" || productcd.val() == "SPR20160308000016415" || productcd.val() == "SPR20160215000015629"){
					        	disabeld = "disabled";
					        }
							
//							if(!MobileOrderBillingStep1.isSnsLogin) {
//								arrHtml.push("<div class='pointBox clearfix'>");
//								arrHtml.push("	<div class='chk_point_use'>");
//								//비회원은 뷰티포인트 결제X
//								if(MobileOrderBillingStep1.isLogin){
//									
//									arrHtml.push("		<span class='inputChk "+disabeld+"'> <input type='checkbox' id='beautyPoing"+i+"' name='i_arrFlagBpoint_temp' class='checkbox' "+disabeld+" /><label for='beautyPoing"+i+"'>뷰티포인트 결제</label></span>");
//									
//								}
//									arrHtml.push("		<select id='i_arrPayBpoint' name='i_arrPayBpoint' class='selectBox3' style='display: none;'>");
//									arrHtml.push("			<option value='0'>0개[0원]</option>");							
//									arrHtml.push("		</select>");
//									
//								arrHtml.push("	</div>");
...
//							}
								...
								if(freegoodlistSize>0){
									...
									for(var k=0; k<freegoodlistSize; k++){
										...
										if(productkey == UproductKey){
											...
											arrGift.push("			<p class='prodNm'>"+freegoodnm.val()+" * "+freegoodcnt.val()+"개</p>");	
					...
					arrHtml.push("		<a href='javascript:;' class='btn_freeGift'>사은품 선택하기</a>");
					arrHtml.push("	</div>");
					target.html($(arrHtml.join("")));
			...
			if(waterSize > 0){
					...
					for(var i=0; i<waterSize; i++){
						...
						MobileOrderBillingStep1.rebon_pointmall_cnt++;
					}
			...
			if(vipSize > 0){
					...
					for(var i=0; i<vipSize; i++){
						...
						MobileOrderBillingStep1.vip_pointmall_cnt++;
					}					
	...
	, setNamingPrice : function (namingcd, namingCnt, namingPrice) {
		...
		MobileOrderBillingStep1.sum_naming_cnt 	= namingCnt;
		MobileOrderBillingStep1.sum_naming_price 	= namingPrice;
		MobileOrderBillingStep1.setPayStats();
	...
	, sum : function (flag) {
		...
		for (var i = 0; i < size; i++) {
			...
			var channel_dc_per			= MobileOrderBillingStep1.channel_dc_per;
			var channel_dc_brd_exc		= MobileOrderBillingStep1.channel_dc_brd_exc;
		    ...
		    if(dayoffStockqty > 0 && dayoffcd !=""){
				...
				if(mPointflag.indexOf("PER_")>-1){
					...
					save_mpoint_per	= MobileOrderBillingStep1.isLogin ? fnOnlyNumber(mpoint_per).number : 0;	
					
				}else{
					
					save_mpoint		= MobileOrderBillingStep1.isLogin ? fnOnlyNumber(mpoint_per).number : 0;
			...
			// [s] 뷰티포인트 사용여부
			var flag_bpoint	= $("input[name='i_arrFlagBpoint']", tr.eq(i)).val();
			
			if (flag_bpoint == "Y") {
				
				if (!MobileOrderBillingStep1.isBpointSetting(tr.eq(i), feild.eq(i), sum_pay_bpoint)) {
					
					showMessageBox({message : "뷰티포인트 잔액이 부족해요.",
						close : function(){

							var el_checkbox = $("input[name='i_arrFlagBpoint_temp']", feild.eq(i));
							el_checkbox.prop("checked", false);
							
							MobileOrderBillingStep1.setBpointCheckEvent(tr.eq(i),feild.eq(i));
						}
					});
			...
			pay_bpoint	= fnOnlyNumber($("*[name='i_arrPayBpoint']", feild.eq(i)).val()).number;
			
			//YHCHOI : 뷰티포인트가 추가되야 함
			bpoint_cnt	= flag_bpoint == "Y" && pay_bpoint > 0 ? pay_bpoint / beauty_price : 0;
		
			MobileOrderBillingStep1.setBpointFreegood($(".span_key", tr.eq(i)).text(), cnt - bpoint_cnt); // 뷰티포인트 결제시 사은품 처리
			// [e] 뷰티포인트 사용여부
			
			if(!isonePlusFlag && typecd !="0003" && !(dayoffStockqty > 0 && dayoffcd !="")){
					...
					if (cnt - bpoint_cnt > 0) {
						...
						for(var j=0; j<cp_info.length; j++){
							...
							if (dis_price == 0 || cp_info[j].v_flag_norpc_apply == "Y") {
								...
								if(dis_price < dis_coupon_price_temp){
									...
									if (cp_info[j].v_flag_beauty == "Y") {
										...
										row_save_bpoint		+= Math.round((couponPrice) * bpoint_per / 100) * cnt_temp;
										
									}else{
										coupon_beauty_flag = "N";
									}
									row_save_mpoint += save_mpoint * (cnt - cnt_temp - bpoint_cnt) + Math.round((couponPrice) * save_mpoint_per / 100) * cnt_temp;
							...
							if (channel_dc_per > 0 && channel_dc_brd_exc.indexOf(brandcd) == -1) {
								...
								if(cp_info[j].v_flag_option == "Y"){
									dis = (price-dis_coupon_price_temp) * (channel_dc_per / 100);
						...
						if(price * cnt_temp <dis_coupon_price){
							dis_coupon_price = price;
						}
						
						row_pay_coupon	+= dis_coupon_price;
						row_pay_price	+= ((price * dis_cp_prod_cnt) - dis_coupon_price);
					}
				}
			}
...
//			}				
			...
			if(isonePlusFlag){
				...
				if("Y" == $("input[name='i_arrFlagBeauty']", tr.eq(i)).val()){
					...
					if(MobileOrderBillingStep1.flag_beauty_hotdeal){
						row_sum_price		 += price  * cnt;
					}else{
				...
				if("Y" == $("input[name='i_arrFlagBeauty']", tr.eq(i)).val() && MobileOrderBillingStep1.flag_beauty_hotdeal != "Y"){
					row_dis_price 			+= 0;
				}else{
			...
			$(".row_save_bpoint", feild.eq(i)).text(MobileOrderBillingStep1.isLogin ? SetNumComma(row_save_bpoint) : 0);
			$(".row_save_mpoint", feild.eq(i)).text(MobileOrderBillingStep1.isLogin ? SetNumComma(row_save_mpoint) : 0);
			
			if(bpoint_per > 0){
				if(cnt - bpoint_cnt == 0 || row_save_bpoint == 0){
					$(".row_save_bpoint_per", feild.eq(i)).text("(0%)");
				}else{
					$(".row_save_bpoint_per", feild.eq(i)).text(MobileOrderBillingStep1.isLogin ? '('+SetNumComma(bpoint_per)+'%)' : 0);
				}
			}
			
			if(save_mpoint_per > 0){
				
				if(cnt - bpoint_cnt == 0 || row_save_mpoint == 0){
					$(".row_save_mpoint_per", feild.eq(i)).text("(0%)");
				}else{
					$(".row_save_mpoint_per", feild.eq(i)).text(MobileOrderBillingStep1.isLogin ? '('+SetNumComma(save_mpoint_per)+'%)' : 0);
				}
			}
		...
		MobileOrderBillingStep1.sum_price 				= sum_price;
		MobileOrderBillingStep1.sum_dis_price 			= sum_dis_price;
		
		MobileOrderBillingStep1.sum_dis_coupon_price 	= sum_dis_coupon_price;
		MobileOrderBillingStep1.sum_pay_price 			= sum_pay_price;
		MobileOrderBillingStep1.sum_dis_channel_price  = sum_dis_channel_price;
		
		MobileOrderBillingStep1.sum_mpoint 			= sum_mpoint;
		MobileOrderBillingStep1.sum_pay_mpoint 		= sum_pay_mpoint;
		MobileOrderBillingStep1.sum_pay_bpoint 		= sum_pay_bpoint;
		
		MobileOrderBillingStep1.sum_save_bpoint		= sum_save_bpoint;
		MobileOrderBillingStep1.sum_save_mpoint		= sum_save_mpoint;
		
		MobileOrderBillingStep1.sum_need_bpoint		= sum_need_bpoint;
		MobileOrderBillingStep1.sum_zero_point_price   = sum_zero_point_price;
		MobileOrderBillingStep1.sum_zero_mpoint_price   = sum_zero_mpoint_price;
		MobileOrderBillingStep1.setDeliveryPrice(false);
...
//				MobileOrderBillingStep1.sum_dis_coupon_price = MobileOrderBillingStep1.delivery_price; 
				MobileOrderBillingStep1.flag_use_delivery_coupon = "Y";
				MobileOrderBillingStep1.delivery_price = 0;
		...
		if (!cpobj.isUsedAll) {
			...
			for (var i = 0; i < size_temp; i++) {
				...
				if(cpobj.data.v_typecd != "0006" && cpobj.data.v_flag_pay_money == "M" && (dis_price >= cpobj.data.n_pay_money || dis_price2 >= cpobj.data.n_pay_money)){
					if(dayoffStockqty > 0 && dayoffcd != ""){
						is_dayoffFlag = true;
					}else{
				...
				if(cpobj.data.v_typecd != "0006" && cpobj.data.v_flag_pay_money == "P" && (dis_per >= cpobj.data.n_pay_money || dis_per2 >= cpobj.data.n_pay_money)){
					if(dayoffStockqty > 0 && dayoffcd != ""){
						is_dayoffFlag = true;
					}else{
						is_high_saleFlag = true;
		...
		MobileOrderBillingStep1.fgSum();
		MobileOrderBillingStep1.setPayStats();
		MobileOrderBillingStep1.GiftList(flag);			
	...
	, setDeliveryPrice : function ( isPayStats ) {
		...
		if (MobileOrderDelivery.cash_buy_limit > 0 
			...
			if (isDeliveryToday && MobileOrderDelivery.cash_today_limit > 0 && MobileOrderDelivery.cash_today_limit <= MobileOrderBillingStep1.sum_pay_price ) {
				isDeliveryFree = true;
			}
			else if (isDeliveryToday && MobileOrderDelivery.point_today_limit > 0 && MobileOrderDelivery.point_today_limit <= MobileOrderBillingStep1.sum_pay_bpoint ) {
				isDeliveryFree = true;
			}
			else if (MobileOrderDelivery.cash_buy_limit > 0 && MobileOrderDelivery.cash_buy_limit <= MobileOrderBillingStep1.sum_pay_price ) {
				isDeliveryFree = true;
			}
			else if (MobileOrderDelivery.point_buy_limit > 0 && MobileOrderDelivery.point_buy_limit <= MobileOrderBillingStep1.sum_pay_bpoint ) {
				isDeliveryFree = true;
			}
			else if(MobileOrderBillingStep1.flag_beauty_hotdeal == "Y"){
				isDeliveryFree = true;
			}
		}
		...
		if (isDeliveryFree) {
			...
			if (isDeliveryToday && MobileOrderBillingStep1.sum_pay_price > 0) {
				MobileOrderBillingStep1.delivery_price = MobileOrderDelivery.cash_today;	
			}
			else if (isDeliveryToday && MobileOrderBillingStep1.sum_pay_bpoint > 0) {
				MobileOrderBillingStep1.delivery_price = MobileOrderDelivery.point_today;
			}
			else if (!isDeliveryToday && MobileOrderBillingStep1.sum_pay_price > 0) {
				MobileOrderBillingStep1.delivery_price = MobileOrderDelivery.cash_buy;
			}
			else if (!isDeliveryToday && MobileOrderBillingStep1.sum_pay_bpoint > 0) {
				MobileOrderBillingStep1.delivery_price = MobileOrderDelivery.point_buy;
			}
			else {
				MobileOrderBillingStep1.delivery_price = MobileOrderDelivery.cash_buy;
			}
	...
	, fgSum : function () {
		...
		MobileOrderBillingStep1.sum_fg_price  = sum_fg_price;
		MobileOrderBillingStep1.sum_fg_mpoint = sum_fg_mpoint;
	}
	, setPayStats : function () {
		
		if (MobileOrderDelivery.naming_limit > 0 && MobileOrderBillingStep1.sum_pay_price >= MobileOrderDelivery.naming_limit) {
			MobileOrderBillingStep1.sum_naming_price = 0;
		}
		
		MobileOrderBillingStep1.sum_pay_money = MobileOrderBillingStep1.sum_pay_price + MobileOrderBillingStep1.delivery_price + MobileOrderBillingStep1.gifgbox_price + MobileOrderBillingStep1.sum_naming_price - MobileOrderBillingStep1.sum_pay_giftcard-MobileOrderBillingStep1.sum_dis_channel_price;
		
		
		$(".sum_price").text(SetNumComma(""+parseInt(MobileOrderBillingStep1.sum_price)));						 // 주문금액
		$(".sum_dis_price").text(SetNumComma(""+ ((MobileOrderBillingStep1.sum_dis_price) * -1)));		 // 할인금액
		$(".dcadd_price").text(SetNumComma(""+MobileOrderBillingStep1.sum_dis_channel_price*-1));
		if(MobileOrderBillingStep1.sum_dis_channel_price > 0){
			$("#chdctitle").show();
			$("#chdcprice").show();
		}else{
			$("#chdctitle").hide();
		...
		$(".sum_need_bpoint").text(SetNumComma(""+ (MobileOrderBillingStep1.sum_need_bpoint)));		 // 필요 뷰티 포인트 금액
		$(".sum_dis_coupon_price").text(SetNumComma(""+ MobileOrderBillingStep1.sum_dis_coupon_price)); //총 쿠폰 할인 금액
		$(".sum_pay_giftcard").text(SetNumComma(""+ (MobileOrderBillingStep1.sum_pay_giftcard + MobileOrderBillingStep1.sum_dis_coupon_price)));// 쿠폰 및 기프트카드 사용금액
		$(".sum_pay_bpoint").text("0");	// 뷰티포인트 결제
		$(".suppl_price").text(SetNumComma("" + (MobileOrderBillingStep1.gifgbox_price + MobileOrderBillingStep1.sum_naming_price) )); //부가 서비스 금액
		$(".delivery_price").text(SetNumComma(""+MobileOrderBillingStep1.delivery_price));	// 배송비
		$(".tot_dcprice").text(SetNumComma(""+parseInt(MobileOrderBillingStep1.sum_dis_price+MobileOrderBillingStep1.sum_dis_channel_price+MobileOrderBillingStep1.sum_dis_coupon_price)));
		
		if(!MobileOrderBillingStep1.isLogin){
			
			if(0>MobileOrderBillingStep1.sum_pay_money){ 
				
				MobileOrderBillingStep1.sum_pay_money = 0;
				
				$(".sum_pay_money").text(""+0);
			}else{
				$(".sum_pay_money").text(SetNumComma(""+MobileOrderBillingStep1.sum_pay_money));								
			}
		...
		$(".sum_pay_mpoint").text(SetNumComma(""+MobileOrderBillingStep1.sum_pay_mpoint));	  // 수분포인트 결제

		$(".sum_save_bpoint").text(MobileOrderBillingStep1.isLogin ? SetNumComma("" + MobileOrderBillingStep1.sum_save_bpoint) : 0);
		$(".sum_save_mpoint").text(MobileOrderBillingStep1.isLogin ? SetNumComma("" + MobileOrderBillingStep1.sum_save_mpoint) : 0);
		
		if(MobileOrderBillingStep1.sum_save_bpoint > 0){
			$(".sum_save_bpoint_per").text("("+Math.round((MobileOrderBillingStep1.sum_save_bpoint * 100)/(MobileOrderBillingStep1.sum_pay_money  + MobileOrderBillingStep1.sum_pay_giftcard- MobileOrderBillingStep1.delivery_price - MobileOrderBillingStep1.sum_naming_price - MobileOrderBillingStep1.gifgbox_price - MobileOrderBillingStep1.sum_zero_point_price + MobileOrderBillingStep1.sum_dis_channel_price)*10)/10+"%)");
		}else{
			$(".sum_save_bpoint_per").text("(0%)");	
		}
		
		if(MobileOrderBillingStep1.sum_save_mpoint > 0){
			$(".sum_save_mpoint_per").text("("+Math.round((MobileOrderBillingStep1.sum_save_mpoint * 100)/(MobileOrderBillingStep1.sum_pay_money  + MobileOrderBillingStep1.sum_pay_giftcard- MobileOrderBillingStep1.delivery_price - MobileOrderBillingStep1.sum_naming_price - MobileOrderBillingStep1.gifgbox_price - MobileOrderBillingStep1.sum_zero_mpoint_price + MobileOrderBillingStep1.sum_dis_channel_price)*10)/10+"%)");
		}else{
		...
		$("input[name='i_iSumPrice']", frm).val(MobileOrderBillingStep1.sum_price + MobileOrderBillingStep1.sum_fg_price);
		
		$("input[name='i_iSumPayPrice']", frm).val(MobileOrderBillingStep1.sum_pay_money);
		
		$("input[name='i_iSumPayBpoint']", frm).val(MobileOrderBillingStep1.sum_pay_bpoint);
		$("input[name='i_iSumSaveBpoint']", frm).val(MobileOrderBillingStep1.sum_save_bpoint);
		$("input[name='i_iSumSaveMpoint']", frm).val(MobileOrderBillingStep1.sum_save_mpoint);
		
		$("input[name='i_iSumNamingPrice']", frm).val(MobileOrderBillingStep1.sum_naming_price);
		
		$("input[name='i_iSumMpoint']", frm).val(MobileOrderBillingStep1.sum_mpoint);
		$("input[name='i_iSumPayMpoint']", frm).val(MobileOrderBillingStep1.sum_pay_mpoint);
		
		$("input[name='i_iSumDisCouponPrice']", frm).val(SetNumComma(MobileOrderBillingStep1.sum_dis_coupon_price));
		
		$("input[name='i_iDeliveryPrice']", frm).val(MobileOrderBillingStep1.delivery_price);

		if (MobileOrderBillingStep1.sum_pay_money == 0) {
			if($("#i_sPayTypecd").val() == "0022"){
				$("#i_sPayTypecd").val("");
			}
		...
		var summary = $(".summary_ord");
		$(".price>em",summary).text(SetNumComma(MobileOrderBillingStep1.sum_pay_money));
	}
};
