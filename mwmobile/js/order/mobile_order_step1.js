...
var MobileOrderCoupon = {
	...
	,addBtnEvent : function(){
		...
		$(".btn_user_coupon_pop").unbind("click").click(function(event){
			...
			var usedAbsCpCnt	= MobileOrderCoupon.getUsedAbsCouponCnt();
			...
			if (usedCpCnt > 0) {
				...
				if (usedAbsCpCnt > 0) {
					showMessageBox({
						message : "쿠폰 사용으로 인해 슈퍼쿠폰 사용이 초기화 되었어요!<br/>다시 적용해주세요"
							, close : function() {
								MobileOrderCoupon.clearAbsCoupon();
		...
		$(".btn_user_abs_coupon_pop").unbind("click").click(function(event){
			...
			var usedCpCnt	= MobileOrderCoupon.getUsedAbsCouponCnt();
			var usedCartCnt	= MobileOrderCoupon.getUsedCartCouponCnt();
			...
			if (usedCpCnt > 0) {
				...
				if(usedCartCnt > 0) {
					showConfirmBox({
						message : "절대쿠폰은 장바구니 쿠폰인[" + MobileOrderCoupon.cart_user_couponnm + "] 쿠폰과 함께 사용하실 수 없어요.<br/>[" + MobileOrderCoupon.cart_user_couponnm + "] 사용을 취소하시겠어요?"
						, ok_func : function() {
							MobileOrderCoupon.clearCoupon("clear");
							MobileOrderCoupon.clearAbsCoupon();
							MobileOrderCoupon.showUserAbsCouponPopup();
						}
						, ok_str : "예"
						, cancel_str : "아니오"
					});
		...
		// TODO :: 체크박스 function
		$("#div_abs_coupon_list").on("click", "input[name='i_arrAbsCheckbox']", function() {
			...
			var arrChk			= $("input[name='i_arrCheckbox']").filter(":checked");
			var arrChkSize		= arrChk.size();
			...
			for (var i = 0; i < arrChkSize; i++) {
				...
				var subtr			= arrChk.eq(i).parents(".tr_coupon_list").eq(0);
				var subtype			= $(".span_typecd",subtr).text();
				var flag_pay_money	= $(".span_flag_pay_money",subtr).text();
			...
			var arrAbsChk		= $("input[name='i_arrAbsCheckbox']").not(this).filter(":checked");
			arrAbsChk.each(function(){
				$(this).prop("checked", false);
				$(this).parents("label").removeClass("active");
			})

			if ($(this).is(":checked")) {
				MobileOrderCoupon.fnApplyAbsTemp(user_couponcd, type);
			}
			else {
				MobileOrderCoupon.fnApplyAbsTemp(user_couponcd, type);
			}

			$("#modalPopupAbsCoupon").contents().find(".content").resize();
		...
		$("#div_abs_coupon_list").on("click",".btn_abs_coupon_apply",function(event) {
			event.preventDefault();
			MobileOrderStep1.setGiftcardClear();
			MobileOrderCoupon.fnAbsApply();
		});
		...
		$("#dl_abs_coupon_list").on("click",".btn_delete_abs_coupon",function(event) {
			event.preventDefault();
			MobileOrderCoupon.clearAbsCoupon();
		});
		/**gypark : AP몰 개선 - 전화번호 입력시, 숫자가 아니면 alert창 뜨게 함 [s] */
	...
	, getUsedCartCouponCnt : function () { // TODO :: getUsedCouponCnt
		var cnt = 0;
		var size = MobileOrderCoupon.list == undefined ? 0 : MobileOrderCoupon.list.length;
		var arrCouponnm = [];
		for (var i = 0; i < size; i++) {
			if (MobileOrderCoupon.list[i].v_flag_use == "Y" && MobileOrderCoupon.list[i].v_typecd == "0004") {
				arrCouponnm.push(MobileOrderCoupon.list[i].v_couponnm);
				cnt++;
			}
		}
	...
	, getUsedAbsCouponCnt : function () { // TODO :: getUsedAbsCouponCnt
		var cnt = 0;
		var size = MobileOrderCoupon.absList == undefined ? 0 : MobileOrderCoupon.absList.length;
		var arrCouponnm = [];
		for (var i = 0; i < size; i++) {
			if (MobileOrderCoupon.absList[i].v_flag_use == "Y") {
				arrCouponnm.push(MobileOrderCoupon.absList[i].v_couponnm);
				cnt++;
			}
		}
		return cnt;
	...
	, clearCoupon : function (flag) {
		...
		$(".div_coupon_freegood").hide();
		$("#dl_coupon_list").html("");
		if(flag == "clear" && MobileOrderStep1.total_apply_bpoint > 0){
	...
	, clearCartCoupon : function () {
		...
		MobileOrderCoupon.cart_user_couponnm	= "";
	...
	, clearAbsCoupon : function () { // TODO :: clearAbsCoupon

		MobileOrderCoupon.absList = undefined;
		var arrChk = $("input[name='i_arrAbsCheckbox']", $("#div_abs_coupon_list"));

		for (var i = 0; i < arrChk.length; i++ ) {
			arrChk.eq(i).prop("checked", false);
			var id = arrChk.eq(i).attr("id");
			$("label[for='"+id+"']").removeClass("active");
			arrChk.eq(i).parents("label").remove("active");
		}

		MobileOrderCoupon.abs_discount_amount	= 0;
		MobileOrderCoupon.abs_user_couponcd		= "";
		MobileOrderCoupon.abs_user_couponnm		= "";
		MobileOrderCoupon.abs_flag_beauty		= "";
		MobileOrderCoupon.abs_flag_norprc		= "";
		MobileOrderCoupon.abs_max_dcprc			= "";
		MobileOrderCoupon.abs_max_ori_dcprc		= "";
		MobileOrderCoupon.abs_flag_brand		= "";
		MobileOrderCoupon.abs_flag_pay_money	= "";

		$("#dl_abs_coupon_list").html("");
	...
	, changeAbsFlagUse : function (user_couponcd, flag) {
		var size = MobileOrderCoupon.absList == undefined ? 0 : MobileOrderCoupon.absList.length;
		for (var i = 0; i < size; i++) {
			if (MobileOrderCoupon.absList[i].v_user_couponcd == user_couponcd) {
				MobileOrderCoupon.absList[i].v_flag_use = flag;
				break;
			}
		}
	}
	...
	, showUserAbsCouponPopup : function () {
		...
		MobileCommon.ajax({
			...
			, success : function ( data, textStatus, jqXHR) {
				if(data.status == "succ"){
					MobileOrderHtml.AbsCouponLayerPopup(data);
				}
			}
		});
	...
	, getCheckCouponUsedAll : function () {
		...
		var absSize = MobileOrderCoupon.absList == undefined ? 0 : MobileOrderCoupon.absList.length;
		for (var i = 0; i < absSize; i++) {
			if(MobileOrderCoupon.absList[i].v_typecd != "0007"){
				if (MobileOrderCoupon.absList[i].n_min_price > pay_price || MobileOrderCoupon.absList[i].v_flag_use == "N") {
					MobileOrderCoupon.absList[i].v_flag_use = "D";
					obj.isUsedAll = false;
					obj.data = MobileOrderCoupon.absList[i];
					break;
				}
				else{
					MobileOrderCoupon.absList[i].v_flag_use == "Y";
				}
			}
		}
	...
	, couponReg : function() {
		...
		if(today >= 201705110000 && (code =="syrup1" || code =="syrup2" || code == "syrup"|| code == "loveap" || code == "luckyap" || code == "saleap" || code=="9loveap" || code=="92loveap")){
			showMessageBox({
			...
			, dataType : "json"
				...
				, success : function(data, textStatus, jqXHR) {
					if(data.status == "succ") {
						...
						var new_cnt			=	data.object.userCouponCnt;
						var new_abs_cnt		=	data.object.userAbsCouponCnt;

						$(".span_coupon_cnt").text(new_cnt);
						$(".span_abs_coupon_cnt").text(new_abs_cnt);
	...
	, fnAbsApply : function () {	// TODO :: fnAbsApply

		var arr_tr2			= $(".tr_coupon_list", $("#div_abs_coupon_list"));
		var arr_abs_chkbox	= $("input[name='i_arrAbsCheckbox']", arr_tr2).not(":disabled");
		var arr_abs_chkbox2	= arr_abs_chkbox.filter(":checked");
		var abs_size		= arr_abs_chkbox.size();
		var abs_size2		= arr_abs_chkbox2.size();
		var absList			= [];

		if (abs_size == 0) {
			showMessageBox({message : "적용하실 수 있는 쿠폰이 없어요."});
			modalPopupClose('#modalPopupAbsCoupon');
			return;
		}

		if (abs_size2 == 0) {
			showMessageBox({
				message : "사용하실 쿠폰을 선택해 주세요."
				,close : function(){
					$("<div class='overlay'></div>").appendTo("#wrap");
				}
			});
		...
		for (var i = 0; i < abs_size2; i++) {

			var tr					= arr_abs_chkbox2.eq(i).parents(".tr_coupon_list").eq(0);
			var cnt					= $(".tr_coupon_list").index(tr)+1;
			...
			var v_flag_pay_money	= $(".span_flag_pay_money",tr).text();						// P - 정률, M - 정액
			var n_min_money			= fnOnlyNumber($(".span_min_money",tr).text()).number;
			var v_discount_text		= $(".span_discount_text",tr).text();
			var v_flag_beauty		= $(".span_flag_beauty",tr).text();
			var v_flag_norpc_apply	= $(".span_flag_norpc_apply",tr).text();
		...
		MobileOrderCoupon.setApplyAbsCoupon(absList, 'apply');
		modalPopupClose('#modalPopupAbsCoupon');
		...
		if(isDisPriceFlag){
			...
			showMessageBox({message : ""+isProductnm+"상품은 현재 할인적용된 상품입니다.<br/><span style='color:red;'>해당 쿠폰 사용시, 정상금액에서 쿠폰할인이 적용됩니다.</sapn>"
				,close : function(){
					...
					$("#amtDcRpc").text(SetNumComma(""+ MobileOrderStep1.sum_dis_coupon_price) + "원");
					//modalPopupClose('#modalPopupCoupon');
				}
			});
	...
	, fnApplyAbsTemp : function (usercpcd, typecd) { // TODO :: fnApplyAbsTemp

		var arr_tr				= $(".tr_coupon_list", $("#div_abs_coupon_list"));
		var arr_abs_chkbox		= $("input[name='i_arrAbsCheckbox']", arr_tr).not(":disabled");
		var arr_abs_chkbox2		= arr_abs_chkbox.filter(":checked");
		var abs_size			= arr_abs_chkbox2.size();
		var absList				= [];

		for (var i = 0; i < abs_size; i++) {
		...
		MobileOrderCoupon.setApplyAbsCoupon(absList, '');
		$("#amtDcRpc2", $("#div_abs_coupon_list")).text(SetNumComma(""+ MobileOrderStep1.sum_dis_abs_coupon_price) + "원");
		...
		for (var i = 0; i < size; i++) {
			...
			if (list[i].v_typecd == "0004" || list[i].v_typecd == "0006") {
				if (list[i].v_flag_pay_money == "P") {
					...
					MobileOrderCoupon.cart_user_couponnm	= MobileOrderCoupon.list[i].v_couponnm
...
//					}
					...
					, success : function ( data, textStatus, jqXHR) {
						...
						if(data.status == "succ"){
							...
							if ($(".tr_order_coupon_freegood").size() == 0) {
								...
								showMessageBox({message : "앗, 죄송합니다.  현재 재고가 모두 소진되어, 현재 주문과 함께 발송이 불가해요."});
								return;
		...
		if(size == 1 && list[0].v_typecd == "0003"){
			...
			if(MobileOrderStep1.total_apply_syrup_point > 0){
				MobileOrderStep1.clearPaySyrupPoint();
				showMessageBox({
					message : "쿠폰 사용으로 인해 두툼포인트 사용이 초기화 되었어요!<br/>다시 적용해주세요"
				});
			}
		}
	...
	, setApplyAbsCoupon : function (list, flag) { // TODO :: setApplyAbsCoupon

		MobileOrderCoupon.absList	= list;

		var size	= MobileOrderCoupon.absList.length;

		if (size > 0) {
			for (var i = 0; i < size; i++) {
				if(list[i].v_typecd == "0007") {
					MobileOrderCoupon.abs_discount_amount	= MobileOrderCoupon.absList[i].n_pay_money;
					MobileOrderCoupon.abs_user_couponcd		= MobileOrderCoupon.absList[i].v_user_couponcd;
					MobileOrderCoupon.abs_flag_beauty		= MobileOrderCoupon.absList[i].v_flag_beauty;
					MobileOrderCoupon.abs_flag_norprc		= MobileOrderCoupon.absList[i].v_flag_norpc_apply;
					MobileOrderCoupon.abs_min_money			= MobileOrderCoupon.absList[i].n_min_money;
					MobileOrderCoupon.abs_max_dcprc			= MobileOrderCoupon.absList[i].n_max_dcprc;
					MobileOrderCoupon.abs_max_ori_dcprc		= MobileOrderCoupon.absList[i].n_max_dcprc;
					MobileOrderCoupon.abs_flag_brand		= "N";
					MobileOrderCoupon.abs_flag_pay_money	= MobileOrderCoupon.absList[i].v_flag_pay_money;
					MobileOrderCoupon.abs_user_couponnm		= MobileOrderCoupon.absList[i].v_couponnm;
				}
			}
		}
		else {
		...
		MobileOrderHtml.CouponSubList(MobileOrderCoupon.absList, "abs");
	...
	, setToggleBtn : function(){
		...
		var absCnt = MobileOrderCoupon.getUsedAbsCouponCnt();
		if (absCnt > 0) {
			$(".btn_user_abs_coupon_pop").text("적용취소");
		}
		else {
			$(".btn_user_abs_coupon_pop").text("쿠폰조회");
		}
	}
};
...
var MobileOrderGiftCard = {
	...
	,addBtnEvent : function(){
		...
		$(".btn_giftcard_search").click(function(event){
			...
			if (MobileOrderGiftCard.balance == 0 && MobileOrderStep1.sum_pay_giftcard == 0) {
...
var MobileOrderSyrupPoint = {
	name : "MobileOrderSyrupPoint"
	,cnt : 0
	,use_cnt : 0
	,balance : 0
	,payment_total : 0
	...
	,focus_target : undefined
	,init : function(){
		MobileOrderSyrupPoint.addBtnEvent();
	}
	,addBtnEvent : function(){
		$("#btn_user_syrup_pop").click(function(event){
			event.preventDefault();
			var obj = $(this);
			var payPrice = MobileOrderStep1.sum_pay_money;
			
			if(obj.hasClass("use")){
				if (!MobileOrderStep1.isLogin) {
					showMessageBox({message : "두툼포인트를 사용하시려면 로그인을 해야해요.<br/>로그인 하시겠어요?"});
					return;
				}
				
				MobileOrderSyrupPoint.is_chk = true;
				
				MobileCommon.ajax({
					url : GLOBAL_WEB_ROOT+"order/order_syrup_api_send_ajax.do"
					, type : "POST"
					, data : {
						i_sFlagPoint : "C"
						, i_iPayPrice : payPrice 
						, i_sFlagOcb : $("input[name='i_sFlagOcb']").val()
						, i_sOcbCardno : $("input[name='i_sOcbCardno']").val()
					}
					, dataType : "json"
					, animation : false
					, success : function ( json, textStatus, jqXHR) {
						if (json.status == "succ") {
							...
							var syrupPoint = json.object.i_sSyrupPoint;
							MobileOrderUserInfo.syrup_point = syrupPoint ;
							MobileOrderHtml.SyrupLayerPopup();
						}
						else if(json.status == "invalid") {
							modalPopup("#modalPopupOkCashBack");
						}else if(json.status == "fail"){
							showMessageBox({
								message : "주문하신 분의 성함과 OK캐쉬백 카드 소유주의 이름이 다릅니다. 본인 주문건에 한해서만 두툼포인트 사용 및 OK캐쉬백 카드번호 인증이 가능합니다." +
										" OK캐쉬백 카드번호 문의는 OK캐쉬백 APP 및 웹페이지에서 확인 바랍니다.<br/>(OK 캐쉬백 고객상담 1599-0512))"
							});
							
							$("input[name='i_sFlagOcb']").val("N");
							$("input[name='i_sOcbCardno']").val("");
							$("input[name='i_sOkCardno1']").val("");
							$("input[name='i_sOkCardno2']").val("");
							$("input[name='i_sOkCardno3']").val("");
							$("input[name='i_sOkCardno4']").val("");
						}else{
				...
				$(".span_remain_syrup_point").text(SetNumComma(MobileOrderUserInfo.syrup_point));
...
//						MobileOrderStep1.clearPaySyrupPoint();
		...
		$("#div_syrup_list").on("click","#btn_syrup_point_apply",function(event){
			event.preventDefault();
			var tr			  = $(".tr_product_list");
			var tr_order	  = $(".div_product_list");
			var size 	 	  = tr.size();
			var use_syrup_point    = fnOnlyNumber($("#i_iUseSyrupPoint_temp").val()).number;
			...
			if(use_syrup_point == 0){
				showMessageBox({message : "보유 포인트 보다 더 많은 포인트는 사용할 수 없습니다."});
				MobileOrderSyrupPoint.is_chk = false;
				return;
			}
			
			if(use_syrup_point > calcul_syrup_point){
				showMessageBox({message : "현재 최대 사용가능한 포인트는 "+calcul_syrup_point+"P 입니다.</br>"+calcul_syrup_point+"P 이하로 입력해주세요."});
				MobileOrderSyrupPoint.is_chk = false;
				return;
			}
			
			if(MobileOrderSyrupPoint.is_chk){
				
				var total_apply_syrup_point = use_syrup_point;
				
				$("#inp_syrup_price").val(SetNumComma(total_apply_syrup_point));
				modalPopupClose("#modalPopupSyrupPay");
				$("#btn_user_syrup_pop").removeClass("use");
				$("#btn_user_syrup_pop").addClass("refund");
				$("#btn_user_syrup_pop").text("포인트취소");
				
				$(".span_remain_syrup_point").text(SetNumComma(SetNumComma(MobileOrderUserInfo.syrup_point-total_apply_syrup_point)));
				$(".sum_need_syrup_point").text(SetNumComma(total_apply_syrup_point));
				
				$(".span_remain_syrup_point").text(SetNumComma(MobileOrderUserInfo.syrup_point-total_apply_syrup_point));
				$("input[name='i_sFlagSyrupPoint']").val("Y");
				MobileOrderStep1.total_apply_syrup_point = total_apply_syrup_point;
				MobileOrderStep1.setGiftcardClear();
				MobileOrderStep1.clearPayBpoint();
//				MobileOrderStep1.clearPaySyrupPoint();
				MobileOrderStep1.sum();
				...
				showMessageBox({message : "두툼포인트 사용에 대해 다시 확인해주세요."});
				MobileOrderSyrupPoint.is_chk = false;
				modalPopupClose("#modalPopupSyrupPay");
				return;
			}
		...
		// 두툼포인트 발급
		$(".btn_syruppoint_reg").click(function(event) {
			event.preventDefault();

			//날짜 시간 체크 rnwoo
			var frm = $("form[name='frm']");
			var syrupId = $("input[name='i_sSyrupId']", frm);
			
			MobileCommon.ajax({
				url : GLOBAL_WEB_ROOT+"order/order_syrup_api_send_ajax.do"
				, type : "POST"
				, data : {
					i_sSyrupId :  syrupId.val()
					, i_sFlagPoint : "R"
				}
				, dataType : "json"
				, animation : false
		...
		$(".btn_okcashback").click(function(){
			event.preventDefault();
			
			if(!MobileOrderSyrupPoint.isValidation()){
				return;
			}
			
			var obj = $("#btn_user_syrup_pop");
			var payPrice = MobileOrderStep1.sum_pay_money -MobileOrderStep1.delivery_price;
			var cashbackCd = $("input[name='i_sOkCardno1']").val()+$("input[name='i_sOkCardno2']").val()+$("input[name='i_sOkCardno3']").val()+$("input[name='i_sOkCardno4']").val();
			
			$("input[name='i_sOcbCardno']").val(cashbackCd);
			if(obj.hasClass("use")){
				if (!MobileOrderStep1.isLogin) {
					showMessageBox({message : "두툼포인트를 사용하시려면 로그인을 해야해요.<br/>로그인 하시겠어요?"});
					return;
				...
				MobileCommon.ajax({
					...
					, success : function ( json, textStatus, jqXHR) {
						if (json.status == "succ") {
							...
							MobileOrderUserInfo.syrup_point = syrupPoint ;
							modalPopupClose("#modalPopupOkCashBack");
							MobileOrderHtml.SyrupLayerPopup();
						}
						else if(json.status == "invalid") {
							modalPopup("#modalPopupOkCashBack");
						}else if(json.status == "notused") {
							$("input[name='i_sOkCardno1']").val("");
							$("input[name='i_sOkCardno2']").val("");
							$("input[name='i_sOkCardno3']").val("");
							$("input[name='i_sOkCardno4']").val("");
...
//					MobileOrderStep1.clearPaySyrupPoint();
		...
		var i_sFlagAssignSyrup = $("input[name=i_sFlagAssignSyrup]");
		if(i_sFlagAssignSyrup.is(":checked") == false){
			showMessageBox({message : "두툼포인트 결제를 위한 정보제공 동의가 필요합니다."});
			isResult = false;
		}
		
		if($("input[name='i_sOkCardno1']").val() == "" || $("input[name='i_sOkCardno2']").val() == "" || $("input[name='i_sOkCardno3']").val() == "" || $("input[name='i_sOkCardno4']").val() == ""){
			showMessageBox({message : "카드번호를 입력해 주세요."});
			isResult = false;
		}
	...
	, lguplusCall : function(vo){
		...
		if(vo.paytypecd == "SC0010"){
			...
			if(vo.paytypecd == "SC0010_1"){
				if(vo.cstmid == "sevenstone"){
					affiliateCardInfo = "11-002055,11-01996,11-01997,11-01732,11-01733,11-01734,11-01735,11-04411";
				}else{
...
//			window.open(orderurl, 'popupPayco', 'top=100, left=300, width=727px, height=512px, resizble=no, scrollbars=yes');
	...
	, actionPayment : function () {
		...
		for (var i = 0; i < absSize; i++) {
			if (MobileOrderCoupon.absList[i].v_flag_use == "Y") {
				input = $("<input type='hidden' name='i_arrUserCouponcd' />").appendTo(frm);
				input.val(MobileOrderCoupon.absList[i].v_user_couponcd);
				
				input2 = $("<input type='hidden' name='i_arrCouponTypecd' />").appendTo(frm);
				input2.val(MobileOrderCoupon.absList[i].v_typecd);
				
				input3 = $("<input type='hidden' name='i_arrCouponProdcd' />").appendTo(frm);
				input3.val(MobileOrderCoupon.absList[i].v_productcd);
			} 
		}
...
//								arrHtml003.push("			</span>");
						...
						if(betweencnt == 0 && specialcnt == 0 && MobileOrderStep1.vip_pointmall_cnt == 0 && MobileOrderStep1.rebon_pointmall_cnt == 0 ){
							...
							//아코디언
							var $accotit = $("#modalPopupFreeGift .tit");
							var $accoCont = $("#modalPopupFreeGift .content");

							if($(".div_coupon_freegood").css("display") != "none"){
								$accoCont.eq(0).css("display","block");
								$accotit.eq(0).addClass("active");
							}else{
								$accoCont.eq(0).css("display","none");
								$accotit.eq(0).removeClass("active");
							}
	...
	, sum : function (flag) {
		...
		var sum_dis_abs_coupon_price	= 0;	// 절대쿠폰 할인금액
		...
		var sum_apply_syrup_point_price  = 0;
		var flag_syrup = $("input[name='i_sFlagSyrupPoint']").val();
		var temp_sum_syrup_point = use_syrup_point;
		
		/* 절대쿠폰 관련 [s] */
		var tot_abs_coupon_dis_amount	= parseInt(MobileOrderCoupon.abs_discount_amount);
		var abs_coupon_flag_beauty		= MobileOrderCoupon.abs_flag_beauty;
		var abs_coupon_min_money		= MobileOrderCoupon.abs_min_money
		var sum_abs_able_pay_price		= 0;
		var abs_coupon_last_index		= -1;
		var priceList					= [];
		/* 절대쿠폰 관련 [e] */
		...
		for (var i = 0; i < size; i++) {
			...
			var row_pay_syrup_point 			= 0;
			var row_pay_syrup_point_cnt		= 0;
			...
			var onplus_sum_syrup_point		= 0;
			var coupon_sum_syrup_point		= 0;
			...
			/* 절대쿠폰 관련 [s] */
			var detail_price_list	= [];
			var detail_price_type	= "normal";
			var flag_beauty		= $("input[name='i_arrFlagBeauty']", tr.eq(i)).val();
			/* 절대쿠폰 관련 [e] */

			//MobileOrderStep1.setBpointFreegood($(".span_key", tr.eq(i)).text(), cnt - bpoint_cnt); // 뷰티포인트 결제시 사은품 처리
			
			var pay_syrup_point = fnOnlyNumber(tr.eq(i).find(".span_syrup_point_temp_price").text()).number;
			var syrup_point_cnt	= pay_syrup_point > 0 ? fnOnlyNumber(tr.eq(i).find(".span_syrup_point_temp_cnt").text()).number : 0;
					...
					if((MobileOrderCoupon.cart_flag_brand == "Y"  && MobileOrderCoupon.cart_out_brandcd.indexOf(brandcd) == -1) || MobileOrderCoupon.cart_flag_brand == "N"){
						...
						if ((dis_price == 0 || MobileOrderCoupon.cart_flag_norprc == "Y") && ("Y" != $("input[name='i_arrFlagBeauty']", tr.eq(i)).val())) {
							...
							for(var j=0; j<dis_cp_prod_cnt; j++){
								...
								if((price - dis_coupon_price) > 0){
									//두툼포인트사용가능
									coupon_sum_syrup_point += (price - dis_coupon_price);
									if(use_syrup_point > 0){
										if(MobileOrderStep1.sum_min_syrup_point_price > 0){
											var solo_prd_price = price - dis_coupon_price;
											var syrup_point_price = Math.round(((solo_prd_price/MobileOrderStep1.sum_apply_syrup_point_price) * use_syrup_point)/10) * 10;
											
											row_pay_syrup_point_cnt += 1;
											row_pay_syrup_point += syrup_point_price;
											
											if(((i == size-1) && (j == dis_cp_prod_cnt-1))//마지막일경우 쿠폰for에서도 마지막 
													|| parseInt(sum_pay_syrup_point_cnt + row_pay_syrup_point_cnt) >= MobileOrderStep1.sum_min_syrup_point_cnt){//사용갯수가 총사용가능 갯수보다 크거나 '같을'경우(마지막 row의 상품이 뷰포전용상품일경우, 이전상품에서 사용뷰포에 맞춰준다)
												var last_sum_syrup_point = parseInt((sum_pay_syrup_point) + row_pay_syrup_point);
												syrup_point_price = parseInt(syrup_point_price + (use_syrup_point - last_sum_syrup_point));
												row_pay_syrup_point += parseInt(use_syrup_point - last_sum_syrup_point);
											}
											
											if((temp_sum_syrup_point > 0 && temp_sum_syrup_point < syrup_point_price) ||  (i == size-1 && j == dis_cp_prod_cnt-1)){
												syrup_point_price = temp_sum_syrup_point;
											}else{
												temp_sum_syrup_point = temp_sum_syrup_point-syrup_point_price;
											}
											
											paySyrupPoint = syrup_point_price;
											
										}
										
										sum_min_syrup_point_cnt += 1;
								...
								if((price - dis_coupon_price) >= 5000 && flag_mix == "Y"){
									...
									if(use_bpoint > 0){
										if(MobileOrderStep1.sum_min_bpoint_price > 0){
											var solo_prd_price = price - dis_coupon_price;
											var bpoint_price = Math.round(((solo_prd_price/MobileOrderStep1.sum_min_bpoint_price) * use_bpoint)/10) * 10;
								...
								if(parseInt(price - dis_coupon_price - paybpoint) < 0 && paybpoint > 0){
									flagminus = true;
								}

								row_cart_discount	+= dis_coupon_price;
								row_pay_coupon		+= dis_coupon_price;
								row_pay_price		+= price - dis_coupon_price - paybpoint - paySyrupPoint;
								
//								if (channel_dc_per > 0 && channel_dc_brd_exc.indexOf(brandcd) == -1) {
//									
//									var dis = (price - dis_coupon_price - paybpoint - paySyrupPoint) * (channel_dc_per / 100);
//									var rounddis = parseInt(Math.round(dis/10)*10);
//									var dischprice = (price-dis_coupon_price - paybpoint - paySyrupPoint) - dis;
//									
...
//								}
								...
								detail_price_list.push({
									detail_price		: price
									, detail_dis_price	: dis_coupon_price + paybpoint + paySyrupPoint
									, cnt				: 1
									, v_flag_beauty		: coupon_beauty_flag
								});
							}
						...
						for(var j=0; j<cp_info.length; j++){
							...
							if (dis_price == 0 || cp_info[j].v_flag_norpc_apply == "Y") {
								...
								if(dis_price < dis_coupon_price_temp){
									...
									if((price - dis_coupon_price) > 0 && use_bpoint ==0){
										//두툼포인트사용가능
										if(cp_info[j].v_flag_option == "Y"){
											coupon_sum_syrup_point += (couponPrice * cnt_temp);
										}else{
											coupon_sum_syrup_point += couponPrice;
										}
										...
										if(use_syrup_point > 0){
											...
											if(MobileOrderStep1.sum_min_syrup_point_price > 0){
												...
												if(cp_info[j].v_flag_option == "Y"){
													row_pay_syrup_point += syrup_point_price * cnt_temp;
													pay_coupon_syrup_point += syrup_point_price * cnt_temp;
													row_pay_syrup_point_cnt += cnt_temp;
													
													if((temp_sum_syrup_point > 0 && temp_sum_syrup_point < syrup_point_price)  || ((i == size-1) && (j == parseInt(cp_info.length-1)))){
														syrup_point_price = temp_sum_syrup_point;
														paySyrupPoint = syrup_point_price;
														row_pay_syrup_point = syrup_point_price * cnt_temp;
														pay_coupon_syrup_point = syrup_point_price * cnt_temp;
													...
													row_pay_syrup_point += syrup_point_price;
													pay_coupon_syrup_point += syrup_point_price;
													row_pay_syrup_point_cnt += 1;
											...
											cp_info[j].v_flag_beauty = "N";
											flag_beauty = "N";
											use_bpoint = 0;
										}
										sum_min_syrup_point_cnt += dis_cp_prod_cnt;
							...
							if (channel_dc_per > 0 && channel_dc_brd_exc.indexOf(brandcd) == -1) {
								...
								if(cp_info[j].v_flag_option == "Y"){
									if(last_rest_bpoint != 0){
										...
										dis = (price-dis_coupon_price_temp - paybpoint-paySyrupPoint) * (channel_dc_per / 100);
										rounddis = parseInt(Math.round(dis/10)*10);
...
*/
							...
							if(cp_info[j].v_flag_option == "Y") {
								if(last_rest_bpoint != 0) {
									...
									if(dis_cp_prod_cnt -1 > 0) {
										detail_price_list.push({
											detail_price		: price
											, detail_dis_price	: dis_coupon_price_temp + paybpoint
											, paybpoint			: paybpoint
											, cnt				: dis_cp_prod_cnt-1
											, v_flag_beauty		: cp_info[j].v_flag_beauty
											, v_flag_option		: cp_info[j].v_flag_option
										});
									}

									detail_price_list.push({
										detail_price		: price
										, detail_dis_price	: dis_coupon_price_temp + paybpoint + last_rest_bpoint
										, paybpoint			: paybpoint + last_rest_bpoint
										, cnt				: 1
										, v_flag_beauty		: cp_info[j].v_flag_beauty
										, v_flag_option		: cp_info[j].v_flag_option
									});
						...
						row_pay_coupon	+= dis_coupon_price;
						row_pay_price	+= ((price * dis_cp_prod_cnt) - dis_coupon_price - pay_coupon_bpoint - pay_coupon_syrup_point);
					}
				}
			}
			...
			if(isonePlusFlag){
				...
				var bpoint_onplus = 0;
				var syrup_point_onplus = 0;
				
				if(obj.length > 0) {

					var remain_bpoint = 0;
					var remain_syrup_point= 0;
					
					for(var j=0; j<obj.length; j++) {
						var onplusbpoint = 0;
						var onplusSyrupPoint = 0;
						
						onplus_sale_price += parseInt(obj[j].price * obj[j].cnt);
						onplus_sum_syrup_point += parseInt(obj[j].price * obj[j].cnt);
						
						if(use_syrup_point > 0 && use_bpoint == 0){
							sum_min_syrup_point_cnt += obj[j].cnt;
							row_origin_pay_price += parseInt(obj[j].price * obj[j].cnt);
							if(MobileOrderStep1.sum_min_syrup_point_price > 0){
								var solo_prd_price = parseInt(obj[j].price) * obj[j].cnt;
								var syrup_point_price = Math.round(((solo_prd_price/MobileOrderStep1.sum_apply_syrup_point_price) * use_syrup_point)/10) * 10;
								
//								if(((i == size-1) && (j == obj.length-1)) 
//										|| parseInt(sum_pay_syrup_point_cnt + row_pay_syrup_point_cnt) >= MobileOrderStep1.sum_min_syrup_point_cnt){
//									var last_sum_syrup_point = parseInt(sum_pay_syrup_point + row_pay_syrup_point);
//									row_pay_syrup_point += use_syrup_point - last_sum_syrup_point;
//									remain_syrup_point = parseInt(use_syrup_point - last_sum_syrup_point);
//									
//								}
								
								if(temp_sum_syrup_point < syrup_point_price || i == size-1){
									syrup_point_price =  temp_sum_syrup_point;
								}else{
									temp_sum_syrup_point = temp_sum_syrup_point-syrup_point_price;
								}
								
								row_pay_syrup_point += syrup_point_price; // 모든상품의 두툼포인트사용을 합하기위해
								syrup_point_onplus += syrup_point_price; // 1+1에서만 두툼포인트사용을 합하기위해
								row_pay_syrup_point_cnt += obj[j].cnt;
								
								onplusSyrupPoint = syrup_point_price/ obj[j].cnt;
								onplusbpoint = 0;
							}
						}
						...
						if(obj[j].price == price){
							...
							if(obj[j].price >= 5000 && flag_mix == "Y"){
								if(use_bpoint > 0 && MobileOrderStep1.sum_min_bpoint_price > 0){
									var solo_prd_price = parseInt(obj[j].price);
									bpoint_price_notone = Math.round(((solo_prd_price/MobileOrderStep1.sum_min_bpoint_price) * use_bpoint)/10) * 10;
...
//							if(remain_bpoint != 0){
//								var dis = parseInt(obj[j].price - (onplusbpoint - remain_bpoint) - onplusSyrupPoint) * (channel_dc_per / 100);
//								var rounddis = parseInt(Math.round(dis/10)*10);
//								var dischprice = parseInt(obj[j].price - onplusbpoint - onplusSyrupPoint) - dis;
//								onplus_dis_ch_price += rounddis * parseInt(obj[j].cnt-1);
//								
//								dis = parseInt(obj[j].price - onplusbpoint - onplusSyrupPoint) * (channel_dc_per / 100);
//								rounddis = parseInt(Math.round(dis/10)*10);
//								dischprice = parseInt(obj[j].price - onplusbpoint - onplusSyrupPoint) - dis;
//								onplus_dis_ch_price += rounddis;
//								
//							}else{
//								var dis = parseInt(obj[j].price - onplusbpoint - onplusSyrupPoint) * (channel_dc_per / 100);
//								var rounddis = parseInt(Math.round(dis/10)*10);
//								var dischprice = parseInt(obj[j].price - onplusbpoint - onplusSyrupPoint) - dis;
...
//						}
						...
						detail_price_type = "oneplus";

						if(remain_bpoint != 0){

							if (obj[j].cnt-1 > 0) {
								detail_price_list.push({
									detail_price		: obj[j].price
									, detail_dis_price	: onplusbpoint - remain_bpoint + onplusSyrupPoint
									, paybpoint			: onplusbpoint - remain_bpoint
									, cnt				: obj[j].cnt-1
									, notonecnt			: nopluscnt
								});
							}
							...
							detail_price_list.push({
								detail_price		: obj[j].price
								, detail_dis_price	: onplusbpoint + onplusSyrupPoint
								, paybpoint			: onplusbpoint
								, cnt				: 1
								, notonecnt			: nopluscnt
							});
						...
						if(parseInt(obj[j].price - onplusbpoint) < 0 && onplusbpoint > 0 && remain_bpoint != 0){
							flagminus = true;
						}
					}
				}
				...
				row_pay_price	+= onplus_sale_price - bpoint_onplus - syrup_point_onplus;
				row_save_mpoint += Math.round((save_mpoint * objCnt.buyCnt) / cnt) * cnt + Math.round((onplus_sale_price - bpoint_onplus - syrup_point_onplus) * save_mpoint_per / 100);
				...
				if("Y" == $("input[name='i_arrFlagBeauty']", tr.eq(i)).val()){
					...
					if(typecd != "0003") {
						...
						var restsyruppoint = 0;
						var remain_syrup_point_norm = 0;
						
						if(use_bpoint ==0 && typecd != "0003"){
							if(use_syrup_point > 0 && MobileOrderStep1.sum_min_syrup_point_price > 0 && restcnt > 0){
								var solo_prd_price = parseInt(sale_price);
								var syrup_point_price = Math.round(((solo_prd_price/MobileOrderStep1.sum_apply_syrup_point_price) * use_syrup_point)/10) * 10;
								
	//							if((i == (size-1)) || parseInt(sum_pay_syrup_point_cnt + row_pay_syrup_point_cnt) >= MobileOrderStep1.sum_min_syrup_point_cnt){
	//								var last_sum_syrup_point = parseInt(sum_pay_syrup_point + syrup_point_price);
	//								syrup_point_price = syrup_point_price + (use_syrup_point - last_sum_syrup_point);
	//								
	//								syrup_point_price += (use_syrup_point - last_sum_syrup_point);
	//								remain_syrup_point_norm = use_syrup_point - last_sum_syrup_point; 
	//							}
								
								if((temp_sum_syrup_point > 0 && temp_sum_syrup_point < (syrup_point_price * restcnt)) || (i == (size-1) && i != 0)){
									syrup_point_price = temp_sum_syrup_point/restcnt;
								}else{
									temp_sum_syrup_point = temp_sum_syrup_point-(syrup_point_price * restcnt);
								}
								
								row_pay_syrup_point += syrup_point_price * restcnt; // 한row상품의 두툼포인트사용을 합하기위해
								row_pay_syrup_point_cnt += restcnt;
								restsyruppoint += syrup_point_price;
								
							}

							sum_min_syrup_point_cnt += restcnt;
	...
	//								restbpoint = restbpoint + (use_bpoint - last_sum_bpoint);
						...
						var restsaleprice = sale_price - restbpoint - restsyruppoint;
						...
						if(remain_bpoint_norm != 0){
							...
							if((sale_price) - parseInt(restbpoint + remain_bpoint_norm) <0 && remain_bpoint_norm != 0){
								flagminus = true;
							}
						}else if(remain_syrup_point_norm != 0){
							row_pay_price += (sale_price * (cnt - dis_cp_prod_cnt -1)) - (restsyruppoint * (restcnt-1));
							row_pay_price += (sale_price) - parseInt(restsyruppoint + remain_syrup_point_norm);
						}else{
							row_pay_price += (sale_price * (cnt - dis_cp_prod_cnt)) - (restbpoint * restcnt) - (restsyruppoint * restcnt);
						}
/*						
						if(dayoffStockqty > 0 && dayoffcd !=""){
							row_save_bpoint		 += 0;
...
//								row_dis_ch_price += rounddis;
//							}else if(remain_syrup_point_norm != 0){
//	//							var dis = restsaleprice*cnt * (channel_dc_per / 100);
//								var dis = restsaleprice * (channel_dc_per / 100);
//								var rounddis = parseInt(Math.round(dis/10)*10);
//								var dischprice = restsaleprice - dis;
//								row_dis_ch_price += rounddis * (cnt - dis_cp_prod_cnt -1);
//								
//								dis = (sale_price - (restsyruppoint+remain_syrup_point_norm)) * (channel_dc_per / 100);
//								rounddis = parseInt(Math.round(dis/10)*10);
//								dischprice = (sale_price - (restsyruppoint+remain_syrup_point_norm)) - dis;
//								row_dis_ch_price += rounddis;
//								
//							}else{
//								var dis = restsaleprice * (channel_dc_per / 100);
...
//						}
						...
						if(remain_bpoint_norm != 0) {
							...
							if (cnt - dis_cp_prod_cnt -1 > 0) {
								detail_price_list.push({
									detail_price		: restsaleprice
									, detail_dis_price	: 0
									, baybpoint			: row_pay_bpoint
									, cnt				: cnt - dis_cp_prod_cnt -1
//								, v_flag_beauty		: coupon_beauty_flag
								});
							}
							...
							detail_price_list.push({
								...
								, detail_dis_price	: restbpoint + remain_bpoint_norm
								, paybpoint			: row_pay_bpoint
								, cnt				: 1
//								, v_flag_beauty		: coupon_beauty_flag
							});
							...
							detail_price_list.push({
								detail_price		: sale_price
								, detail_dis_price	: restsyruppoint + remain_syrup_point_norm
								, paybpoint			: 0
								, cnt				: 1
							});
							...
							if (cnt - dis_cp_prod_cnt > 0) {
								detail_price_list.push({
									detail_price		: restsaleprice
									, detail_dis_price	: 0
									, paybpoint			: row_pay_bpoint
									, cnt				: cnt - dis_cp_prod_cnt
//								, v_flag_beauty		: coupon_beauty_flag
								});
							}
						}
						...
						var setOptionList = $(".tr_set_option_list", tr.eq(i));

						setOptionList.each(function(){
							var option_price 					= fnOnlyNumber($(".span_option_price",		$(this)).text()).number;			// 정상가
							var option_sale_price				= fnOnlyNumber($(".span_option_sale_price",	$(this)).text()).number;			// 할인가
							var option_flag_beauty				= (option_price - option_sale_price) == 0 ? "Y" : "N";

							row_pay_price += option_sale_price * cnt;
							row_sum_price += option_price * cnt;

							if(MobileOrderUserInfo.levelcd == "LV15"
							|| MobileOrderUserInfo.levelcd == "LV16"
							|| MobileOrderUserInfo.levelcd == "LV17"
							|| MobileOrderUserInfo.levelcd == "LV18"
							|| MobileOrderUserInfo.levelcd == "LV19"
							|| MobileOrderUserInfo.levelcd == "LV20") {

								var saleRate = parseInt((option_price - option_sale_price) * 100 / option_price);
								if(saleRate == 10) {
									option_flag_beauty = "Y";
								}

							}

							detail_price_list.push({
								detail_price		: option_price
								, detail_dis_price	: option_price - option_sale_price
								, cnt				: cnt
								, v_flag_beauty		: option_flag_beauty
							});
						});
					}
			...
			if((row_pay_price < 0 || flagminus) && row_pay_bpoint > 0){
				showMessageBox({
					message : "0보다 적은 가격으로 계산되는 상품이 존재하여 <br/>사용뷰티포인트를 조정합니다."
					, close : function(){
						MobileOrderStep1.total_apply_bpoint = MobileOrderStep1.total_apply_bpoint - 100;
						$("#inp_bpoint_price").val(SetNumComma(MobileOrderStep1.total_apply_bpoint));
						MobileOrderStep1.sum();
						return;
					}
				});
			...
			if(typecd !="0003"){
				var feild_syrup_point = tr.eq(i).find(".span_syrup_point_temp_cnt");
				if(row_dis_price > 0 || dis_cp_prod_cnt >0 || isonePlusFlag){
					if(isonePlusFlag){
						//1+1일경우에는 1+1적용금액이 두툼사용가능금액
						sum_min_syrup_point_price += (onplus_sum_syrup_point/100)*15;
						sum_apply_syrup_point_price += onplus_sum_syrup_point;
//						alert("1 = " + sum_min_syrup_point_price);
					}else if(dis_cp_prod_cnt > 0){
						//쿠폰썼을경우 쿠폰금액만큼 제한 금액이 사용가능금액
						var rest_norm_cnt = parseInt(cnt - dis_cp_prod_cnt);
						sum_min_syrup_point_price += (coupon_sum_syrup_point/100)*15;
						sum_apply_syrup_point_price += coupon_sum_syrup_point;
//						alert("2 = " + sum_min_syrup_point_price);
						if(rest_norm_cnt > 0){
							sum_min_syrup_point_price += ((sale_price * (cnt - dis_cp_prod_cnt))/100)*15;
							sum_apply_syrup_point_price += sale_price * (cnt - dis_cp_prod_cnt);
//							alert("3 = " + sum_min_syrup_point_price);
						}
						
					}else{
						//쿠폰을 안썼을 경우에는 기본할인금액을 제한 금액이 사용가능금액
						sum_min_syrup_point_price += (((sale_price) * (cnt))/ 100)*15;
						sum_apply_syrup_point_price += sale_price * cnt;
//						alert("4 = " + sum_min_syrup_point_price);
						feild_syrup_point.text(cnt);
					}
				}else{
					sum_min_syrup_point_price += ((price * (cnt))/100)*15;
					sum_apply_syrup_point_price += price * cnt;
					feild_syrup_point.text(cnt);
				}
			}
			...
			//두툼포인트 결제시 뷰포적립안함
			if($("input[name='i_sFlagSyrupPoint']").val() == "Y"){
				row_save_bpoint = 0;
				bpoint_per = 0;
				
				if(row_pay_syrup_point != 0){
					$("input[name='i_arrPaySyrupPoint']", tr.eq(i)).val(row_pay_syrup_point);
					$("input[name='i_arrPaySyrupPointCnt']", tr.eq(i)).val(row_pay_syrup_point_cnt);
					MobileOrderStep1.setBpointFreegood($(".span_key", tr.eq(i)).text(), cnt - row_pay_syrup_point_cnt);
				}
			}
...
*/
			var flag_row_abs_coupon	= ((MobileOrderCoupon.abs_flag_brand == "Y" && MobileOrderCoupon.abs_out_brandcd.indexOf(brandcd) == -1) || MobileOrderCoupon.abs_flag_brand == "N") ? true : false;
			...
			sum_pay_syrup_point 	+= row_pay_syrup_point;
...
//			sum_dis_channel_price   += row_dis_ch_price;
			...
			sum_abs_able_pay_price	+= flag_row_abs_coupon ? (row_pay_price - row_need_bpoint) : 0;									// 절대쿠폰 적용이 불가한 브랜드 제품을 제외한 나머지 결제금액 합계
			abs_coupon_last_index	= (flag_row_abs_coupon && flag_beauty == "N" && (row_pay_price - row_need_bpoint) > 0) ? i : abs_coupon_last_index;	// 절대쿠폰 적용이 가능한 상품의 마지막 index ( 뷰티포인트 상품 제외 )

			priceList.push({
				row_list_price				: price
				, row_sale_price			: sale_price
				, row_sum_price				: row_sum_price
				, row_pay_price				: row_pay_price
				, row_need_bpoint			: row_need_bpoint
				, row_dis_coupon_price		: row_dis_coupon_price
				, row_pay_bpoint			: row_pay_bpoint
				, row_pay_bpoint_only		: row_pay_bpoint_only
				, row_save_bpoint			: row_save_bpoint
				, row_cart_discount			: row_cart_discount
				, row_dis_price				: row_dis_price
				, row_dis_ch_price			: row_dis_ch_price
				, row_pay_bpoint_cnt		: row_pay_bpoint_cnt
				, bpoint_per				: bpoint_per
				, save_mpoint_per			: save_mpoint_per
				, sum_zero_point_price		: sum_zero_point_price
				, flag_onplus				: isonePlusFlag
				, restbuyprice_onplus		: restbuyprice_onplus
				, brandcd					: brandcd
				, detail_price_type			: detail_price_type
				, detail_price_list			: detail_price_list
				, flag_beauty				: flag_beauty
				, dayoffStockqty			: dayoffStockqty
				, dayoffcd					: dayoffcd
				, flag_row_abs_coupon		: flag_row_abs_coupon
				, typecd					: typecd
				, prod_cnt					: cnt
			});
		...
		for(var k = 0; k < priceList.length; k++) {
			var priceObj = priceList[k];
			console.log(priceObj)
			var priceDetailList	= priceObj.detail_price_list;
			if (priceDetailList.length > 0) {
				for (var l = 0; l < priceDetailList.length; l++) {
					var priceDetailObj = priceDetailList[l];
				}
			}
		}
		console.log("######### 전 #########");
		...
		/* 절대쿠폰 계산 [s] */
		// TODO :: 제외브랜드 처리할 것.
		if(tot_abs_coupon_dis_amount > 0 && sum_abs_able_pay_price >= tot_abs_coupon_dis_amount && sum_abs_able_pay_price >= abs_coupon_min_money) {

			var resultList					= [];
			var sum_abs_coupon_price_temp	= 0;	// 상품row의 절대쿠폰 적용합계
			var remain_abs_coupon_price		= tot_abs_coupon_dis_amount;
			var temp_sum_price				= sum_pay_price;
			...
			for(var k = 0; k < priceList.length; k++) {
				...
				var priceObj				= priceList[k];
				var priceDetailList			= priceObj.detail_price_list;
				var priceDetailReturnList	= [];

				var row_pay_price			= priceObj.row_pay_price;
				var row_abs_coupon_price	= 0;

				var saleRate				= parseInt((priceObj.row_list_price - priceObj.row_sale_price) * 100 / priceObj.row_list_price)

				if(priceObj.flag_row_abs_coupon) {

					var row_abs_coupon_dis_price		= 0;
					var sum_row_abs_coupon_dis_price	= 0;
					...
					if (priceDetailList.length > 0) {
						...
						for (var l = 0; l < priceDetailList.length; l++) {
							...
							if((l == priceDetailList.length - 1) && (k == abs_coupon_last_index)) {
								row_abs_coupon_dis_price = remain_abs_coupon_price;
							}
							else {
								row_abs_coupon_dis_price = Math.round((((priceDetailObj.detail_price - priceDetailObj.detail_dis_price) * priceDetailObj.cnt) / temp_sum_price) * tot_abs_coupon_dis_amount / 10) * 10;
							}

							remain_abs_coupon_price			= remain_abs_coupon_price - row_abs_coupon_dis_price;
							sum_row_abs_coupon_dis_price	+= row_abs_coupon_dis_price;

							priceDetailReturnList.push({
								detail_price			: priceDetailObj.detail_price
								, detail_dis_price		: priceDetailObj.detail_dis_price
								, abs_coupon_dis_price	: row_abs_coupon_dis_price
								, cnt					: priceDetailObj.cnt
								, v_flag_beauty			: (typeof priceDetailObj.v_flag_beauty == "undefined") ? ((saleRate != 10 && saleRate != 0) ? "N" : "Y") : priceDetailObj.v_flag_beauty
								, v_flag_option			: (typeof priceDetailObj.v_flag_option == "undefined") ? "N" : priceDetailObj.v_flag_option
								, notonecnt				: (typeof priceDetailObj.notonecnt == "undefined") ? 1 : priceDetailObj.notonecnt
							});

						}

						sum_dis_abs_coupon_price	+= sum_row_abs_coupon_dis_price;
						row_pay_price				= priceObj.row_pay_price - sum_row_abs_coupon_dis_price;

					}

					sum_pay_price	+= row_pay_price - priceObj.row_need_bpoint;

				}

				resultList.push({
					row_list_price				: priceObj.row_list_price
					, row_sale_price			: priceObj.row_sale_price
					, row_sum_price				: priceObj.row_sum_price
					, row_pay_price				: row_pay_price
					, row_need_bpoint			: priceObj.row_need_bpoint
					, row_dis_abs_coupon_price	: sum_row_abs_coupon_dis_price
					, row_dis_coupon_price		: priceObj.row_dis_coupon_price
					, row_pay_bpoint			: priceObj.row_pay_bpoint
					, row_pay_bpoint_only		: priceObj.row_pay_bpoint_only
					, row_save_bpoint			: priceObj.row_save_bpoint
					, row_cart_discount			: priceObj.row_cart_discount
					, row_dis_price				: priceObj.row_dis_price
					, row_dis_ch_price			: priceObj.row_dis_ch_price
					, row_pay_bpoint_cnt		: priceObj.row_pay_bpoint_cnt
					, bpoint_per				: priceObj.bpoint_per
					, save_mpoint_per			: priceObj.save_mpoint_per
					, sum_zero_point_price		: priceObj.sum_zero_point_price
					, flag_onplus				: priceObj.flag_onplus
					, restbuyprice_onplus		: priceObj.restbuyprice_onplus
					, channel_dc_list			: priceObj.channel_dc_list
					, brandcd					: priceObj.brandcd
					, detail_price_type			: priceObj.detail_price_type
					, detail_price_list			: priceDetailReturnList
					, flag_beauty				: priceObj.flag_beauty
					, typecd					: priceObj.typecd
					, prod_cnt					: priceObj.prod_cnt
				});
		...
		for(var k = 0; k < priceList.length; k++) {
			...
			/* 채널할인 계산 관련 [s] */
			var row_dis_ch_price			= 0;
			var row_dis_abs_coupon_price	= (typeof priceObj.row_dis_abs_coupon_price != "undefined") ? priceObj.row_dis_abs_coupon_price : 0;
//			var tot_row_dis_ch_price		= (priceObj.row_sale_price - row_dis_abs_coupon_price) * (channel_dc_per / 100);
//			var tot_rounddis				= parseInt(Math.round(tot_row_dis_ch_price/10)*10) * priceDetailObj.cnt;

			for(var l=0; l < priceDetailList.length; l++) {

				var priceDetailObj			= priceDetailList[l];
				var abs_coupon_dis_price	= (typeof priceDetailObj.abs_coupon_dis_price == "undefined") ? 0 : priceDetailObj.abs_coupon_dis_price;

				if(priceObj.flag_onplus) {

					if(priceDetailObj.notonecnt > 0){
						row_save_bpoint	+= Math.round(((priceDetailObj.detail_price - priceDetailObj.detail_dis_price) * priceDetailObj.cnt - abs_coupon_dis_price) * priceObj.bpoint_per / 100);
					}
					else{
						row_save_bpoint	+= 0;
						sum_zero_point_price += (priceDetailObj.detail_price - priceDetailObj.detail_dis_price) * priceDetailObj.cnt - abs_coupon_dis_price;
					}

				}
				else {

					if(!isEmpty(priceObj.dayoffcd) && priceObj.dayoffStockqty > 0) {
						row_save_bpoint			+= 0;
						row_save_mpoint			+= 0;
						sum_zero_point_price	+= priceDetailObj.detail_price;
						sum_zero_mpoint_price	+= priceDetailObj.detail_price;
					}
					else {

						if(priceObj.row_list_price - priceObj.row_sale_price > 0) {
							...
							|| MobileOrderUserInfo.levelcd == "LV20") {
								...
								if(saleRate == 10) {
									if(typeof priceDetailObj.v_flag_beauty == "undefined" || priceDetailObj.v_flag_beauty == "Y" ) {
//										row_save_bpoint	+= Math.round(((priceDetailObj.detail_price - priceDetailObj.detail_dis_price) * priceDetailObj.cnt - abs_coupon_dis_price) * priceObj.bpoint_per / 100);
										row_save_bpoint	+= Math.round((((priceDetailObj.detail_price - priceDetailObj.detail_dis_price) * priceDetailObj.cnt - abs_coupon_dis_price)) / priceDetailObj.cnt * priceObj.bpoint_per / 100) * priceDetailObj.cnt;
									}
									else {
										row_save_bpoint		+= 0;
										if(priceDetailObj.detail_dis_price <= 0
										|| (typeof priceDetailObj.v_flag_beauty != "undefined" && priceDetailObj.v_flag_beauty == "N" )) {
											sum_zero_point_price += (priceDetailObj.detail_price - priceDetailObj.detail_dis_price) * priceDetailObj.cnt - abs_coupon_dis_price;
										}
									}
								}
								else {
									if(priceObj.row_save_bpoint > 0
									&& (typeof priceDetailObj.v_flag_beauty != "undefined" && priceDetailObj.v_flag_beauty == "Y" )) {
										row_save_bpoint	+= Math.round((((priceDetailObj.detail_price - priceDetailObj.detail_dis_price) * priceDetailObj.cnt - abs_coupon_dis_price)) / priceDetailObj.cnt * priceObj.bpoint_per / 100) * priceDetailObj.cnt;
//										row_save_bpoint	+= Math.round(((priceDetailObj.detail_price - priceDetailObj.detail_dis_price) * priceDetailObj.cnt - abs_coupon_dis_price) * priceObj.bpoint_per / 100);
									}
									else {
...
//									row_save_bpoint	+= Math.round(((priceDetailObj.detail_price - priceDetailObj.detail_dis_price) * priceDetailObj.cnt - abs_coupon_dis_price) * priceObj.bpoint_per / 100);
							...
							if ((typeof priceDetailObj.v_flag_beauty != "undefined" && priceDetailObj.v_flag_beauty == "N")
							|| priceDetailObj.v_flag_beauty == "undefined" ) {
								row_save_bpoint += 0;
								sum_zero_point_price += (priceDetailObj.detail_price - priceDetailObj.detail_dis_price) * priceDetailObj.cnt - abs_coupon_dis_price;
							}
							else {
...
//								row_save_bpoint	+= Math.round(((priceDetailObj.detail_price - priceDetailObj.detail_dis_price) * priceDetailObj.cnt - abs_coupon_dis_price) * priceObj.bpoint_per / 100);
				...
				/* 채널할인 적용 [s] */
				if (channel_dc_per > 0 && channel_dc_brd_exc.indexOf(priceObj.brandcd) == -1) {
					var abs_coupon_dis_price	= typeof priceDetailObj.abs_coupon_dis_price == "undefined" ? 0 : priceDetailObj.abs_coupon_dis_price;
					var tot_row_dis_ch_price	= (priceObj.row_pay_price / priceObj.prod_cnt) * (channel_dc_per / 100);
					var tot_rounddis			= parseInt(Math.round(tot_row_dis_ch_price/10)*10) * priceObj.prod_cnt;
					if (priceObj.typecd == "0003") {
						if(l == (priceDetailList.length - 1)) {
							var last_row_dis_ch_price = tot_rounddis - row_dis_ch_price;
							row_dis_ch_price += last_row_dis_ch_price;
						}
						else {
							var dis = priceDetailObj.cnt == 0 ? 0 : ((priceDetailObj.detail_price - priceDetailObj.detail_dis_price) * priceDetailObj.cnt - abs_coupon_dis_price) / priceDetailObj.cnt * (channel_dc_per / 100);
							var rounddis = parseInt(Math.round(dis/10)*10);
							row_dis_ch_price += (rounddis * priceDetailObj.cnt);
						}
					}
					else {
						var dis = priceDetailObj.cnt == 0 ? 0 : ((priceDetailObj.detail_price - priceDetailObj.detail_dis_price) * priceDetailObj.cnt - abs_coupon_dis_price) / priceDetailObj.cnt * (channel_dc_per / 100);
...
//					row_dis_ch_price += (rounddis * priceDetailObj.cnt);
			...
			$(".row_sale_price",	$(".div_product_list").eq(k)).text(SetNumComma(priceObj.row_pay_price));
			$(".row_save_bpoint",	$(".div_product_list").eq(k)).text(MobileOrderStep1.isLogin ? SetNumComma(row_save_bpoint) : 0);
			$(".row_save_mpoint",	$(".div_product_list").eq(k)).text(MobileOrderStep1.isLogin ? SetNumComma(row_save_mpoint) : 0);
			$("input[name='i_arrPayAbsCouponPrice']",	$(".div_product_list").eq(k)).val((typeof priceObj.row_dis_abs_coupon_price == "undefined") ? 0 : priceObj.row_dis_abs_coupon_price);

			if(priceObj.bpoint_per > 0){
				if(row_save_bpoint == 0){
					$(".row_save_bpoint_per", $(".div_product_list").eq(k)).text("(0%)");
				}else{
					$(".row_save_bpoint_per", $(".div_product_list").eq(k)).text(MobileOrderStep1.isLogin ? '('+SetNumComma(priceObj.bpoint_per)+'%)' : 0);
				}
			}

			if(row_save_mpoint > 0){
				if(priceObj.row_save_mpoint == 0){
					$(".row_save_mpoint_per", $(".div_product_list").eq(k)).text("(0%)");
				}else{
					$(".row_save_mpoint_per", $(".div_product_list").eq(k)).text(MobileOrderStep1.isLogin ? '('+SetNumComma(priceObj.save_mpoint_per)+'%)' : 0);
				}
			}

		}
		console.log("############ 후 [e] ############");
		/* 뷰티 포인트 적립률, 채널할인 계산 [e] */
		...
		MobileOrderStep1.sum_dis_abs_coupon_price	= sum_dis_abs_coupon_price;
		...
		MobileOrderStep1.sum_apply_syrup_point_price	= sum_apply_syrup_point_price;
		...
		$("input[name='i_iMaxUseSyrupPointCnt']").val(MobileOrderStep1.sum_min_syrup_point_cnt);
...
//				MobileOrderStep1.sum_dis_coupon_price = MobileOrderStep1.delivery_price; 
		...
		if (MobileOrderCoupon.abs_discount_amount > 0 && sum_dis_abs_coupon_price > 0) {
			MobileOrderCoupon.changeAbsFlagUse(MobileOrderCoupon.abs_user_couponcd, "Y");
		}
	...
	, setPayStats : function () {
		...
		$(".sum_dis_abs_coupon_price").text(SetNumComma(""+ MobileOrderStep1.sum_dis_abs_coupon_price)); //총 쿠폰 할인 금액
		$(".sum_pay_giftcard").text(SetNumComma(""+ (MobileOrderStep1.sum_pay_giftcard + MobileOrderStep1.sum_dis_coupon_price + MobileOrderStep1.sum_dis_abs_coupon_price)));// 쿠폰 및 기프트카드 사용금액
		...
		$(".tot_dcprice").text(SetNumComma(""+parseInt(MobileOrderStep1.sum_dis_price+MobileOrderStep1.sum_dis_channel_price+MobileOrderStep1.sum_dis_coupon_price + MobileOrderStep1.sum_dis_abs_coupon_price)));
		...
		$("input[name='i_iSumDisCouponPrice']", frm).val(SetNumComma(MobileOrderStep1.sum_dis_coupon_price + MobileOrderStep1.sum_dis_abs_coupon_price));
...
//		$("input[name='i_iSumMinusPrice']", frm).val(MobileOrderStep1.sum_dis_price + MobileOrderStep1.sum_dis_coupon_price+MobileOrderStep1.sum_dis_channel_price);
		...
		$("input[name='i_iSumPriceForSyrup']", frm).val(MobileOrderStep1.sum_apply_syrup_point_price);
