...
var MobileOrderCoupon = {
	...
	, couponReg : function() {
		...
		if(today >= 201705110000 && (code =="syrup1" || code =="syrup2" || code == "syrup" || code == "loveap" || code == "luckyap" || code == "saleap")){
			showMessageBox({
	...
	, fnApplyAbsTemp : function (usercpcd, typecd) { // TODO :: fnApplyAbsTemp
		...
		for (var i = 0; i < abs_size; i++) {
			...
			var tr					= arr_abs_chkbox.eq(i).parents(".tr_coupon_list").eq(0);
			var cnt					= $(".tr_coupon_list").index(tr)+1;
	...
	, lguplusCall : function(vo){
		...
		if(vo.paytypecd == "SC0010"){
			...
			if(vo.paytypecd == "SC0010_1"){
				affiliateCardInfo = "11-002055,11-020550";
				affiliateCardOnly = "Y";
				vo.subtypecd = "11";
					...
					if((MobileOrderCoupon.cart_flag_brand == "Y"  && MobileOrderCoupon.cart_out_brandcd.indexOf(brandcd) == -1) || MobileOrderCoupon.cart_flag_brand == "N"){
						...
						if ((dis_price == 0 || MobileOrderCoupon.cart_flag_norprc == "Y") && ("Y" != $("input[name='i_arrFlagBeauty']", tr.eq(i)).val())) {
							...
							detail_price_list.push({
								detail_price		: price * dis_cp_prod_cnt
								, detail_dis_price	: row_pay_coupon + row_pay_bpoint
								, cnt				: 1
						...
						if(remain_bpoint != 0){
							...
							if (obj[j].cnt-1 > 0) {
								detail_price_list.push({
									...
									, detail_dis_price	: onplusbpoint - remain_bpoint
				...
				if (channel_dc_per > 0 && channel_dc_brd_exc.indexOf(priceObj.brandcd) == -1) {
					...
					if (priceObj.typecd == "0003") {
						if(l == (priceDetailList.length - 1)) {
							...
							console.log("last_row_dis_ch_price		" + last_row_dis_ch_price);
							console.log("tot_row_dis_ch_price		" + tot_row_dis_ch_price);
							console.log("row_dis_ch_price			" + row_dis_ch_price);
							...
							console.log("row_dis_ch_price		" + rounddis * priceDetailObj.cnt)
							console.log("sum_row_dis_ch_price	" + row_dis_ch_price)
						}
					}
					else {
						var dis = priceDetailObj.cnt == 0 ? 0 : ((priceDetailObj.detail_price - priceDetailObj.detail_dis_price) * priceDetailObj.cnt - abs_coupon_dis_price) / priceDetailObj.cnt * (channel_dc_per / 100);
...
//					row_dis_ch_price += (rounddis * priceDetailObj.cnt);
//					
//					console.log("1 = " + (priceDetailObj.detail_price - priceDetailObj.detail_dis_price));
//					console.log("2 = " + ((priceDetailObj.detail_price - priceDetailObj.detail_dis_price) * priceDetailObj.cnt - abs_coupon_dis_price));
//					console.log("3 = " + ((priceDetailObj.detail_price - priceDetailObj.detail_dis_price) * priceDetailObj.cnt - abs_coupon_dis_price) / priceDetailObj.cnt * (channel_dc_per / 100));
//					console.log("4 = " + Math.round(dis/10));
//					console.log("5 = " + Math.round(dis/10)*10);
//					console.log("채널할인 :: " + (((priceDetailObj.detail_price - priceDetailObj.detail_dis_price) * priceDetailObj.cnt - abs_coupon_dis_price) - (rounddis * priceDetailObj.cnt)));
				}
				/* 채널할인 적용 [e] */
